...
// smoke's coverage. Both resolve the SAME Dimensions singleton via `@symbiote-native/engine`, so the
// fake seeds them all.
...
let Dimensions: typeof import('@symbiote-native/engine').Dimensions;
let PixelRatio: typeof import('@symbiote-native/engine').PixelRatio;
...
beforeEach(async () => {
  ...
  vi.resetModules();
  ({ Dimensions, PixelRatio } = await import('@symbiote-native/engine'));
  ({ useWindowDimensions } =
