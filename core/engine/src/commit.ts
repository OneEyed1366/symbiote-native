...
function renderableChildren(node: ISymbioteNode): readonly ISymbioteNode[] {
  ...
  for (const child of node.children) {
    if (isAnchor(child)) {
      // An anchor is flattened away here and never reaches reconcile, so this is the only place
      // that can clear its dirty flag. Leaving it set would be a silent stale-UI bug: markDirty
      // stops at the first dirty ancestor, so a permanently-dirty anchor would swallow every
      // later mark from its subtree and the real parent would never learn anything changed.
      child.dirty = false;
