...
function makeBranch(testID: string): { root: ISymbioteNode; leaf: ISymbioteNode } {
...
describe('the commit walk skips a subtree nothing touched', () => {
  ...
  it('reuses the untouched branch handle by reference across that commit', () => {
    expect(findByTestID(fabric.appRoot(), 'untouched')).toBe(untouchedHandleAtMount);
  });
...
describe('every mutation entry point marks its subtree dirty', () => {
  ...
  it('setText reaches Fabric', () => {
    ...
    surface.commit();
    expect(findByTestID(fabric.appRoot(), 'label')!.children[0].props.text).toBe('after');
  });
