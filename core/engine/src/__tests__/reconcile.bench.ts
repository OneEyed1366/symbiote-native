...
function remount(surface: SymbioteSurface, rootTag: number, rowCount: number): void {
  fabric.reset();
  disposeRoot(rootTag);
  surface.clear();
  const table = createElement('RCTView');
...
describe('krausest operations', () => {
  ...
  bench('create 1000 rows', () => remount(create1kSurface, CREATE_1K_ROOT, 1000), NO_WARMUP);
  ...
  bench(
    ...
    () => {
      if (detachedRow === undefined) {
        ...
        insertBefore(removeTable.table, detachedRow, removeTable.table.children[REMOVE_INDEX]);
  ...
  bench('create 10 000 rows', () => remount(create10kSurface, CREATE_10K_ROOT, 10_000), NO_WARMUP);
...
describe('commit with nothing mutated', () => {
  ...
  bench('no-op commit, 9761 nodes', () => idleScreen.surface.commit(), KRAUSEST_WARMUP);
});
