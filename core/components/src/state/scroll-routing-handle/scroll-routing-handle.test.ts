// STALE-DUPLICATE CANDIDATE (found during a coverage sweep, not deleted here): this file's target,
// ./index.ts, is currently UNREACHABLE from any real import. Every production importer of the
// routing-tail type (state/section-list.ts, state/virtualized-list.ts) resolves the bare specifier
// `./scroll-routing-handle` to the sibling FLAT file `../scroll-routing-handle.ts` — Node/TS module
// resolution tries the exact file before it tries a same-named directory's index — so this folder's
// index.ts is a byte-identical duplicate (only the relative-import depth differs) that nothing ever
// executes in production. The canonical, live copy of this test is `../scroll-routing-handle.test.ts`.
// Flagging for cleanup, not removing: see this task's report for the collision finding.
//
