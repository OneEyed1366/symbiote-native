...
//
// Vue-shape difference from React (flag per the task brief): React's twin takes the RESOLVED
// `navigatorHandle: INavigatorHandle` directly, because a React hook is called fresh on EVERY
// render with the caller's current value, and useImperativeHandle's ref assignment (a layout
// effect) is guaranteed to run before this hook's OWN mount effect, so `handleRef.current` is
// already populated by the time it's read. Vue's setup() runs exactly ONCE per component instance
// — there is no "fresh call every render" to re-supply a resolved value, and a template/h() `ref`
// only resolves to Stack's expose()d handle AFTER mount, so an eagerly-resolved argument evaluated
...
// (children before parents), which gives the same "already populated" guarantee React's layout-
// effect ordering does.
