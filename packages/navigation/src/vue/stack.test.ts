...
// pop through the imperative handle. Stack is imported from './stack' (NOT the package barrel,
...
describe('Vue Stack navigator', () => {
  ...
  it('passes route.params through to the screen component via navigation.push', async () => {
    let receivedParams: unknown;
    function ParamsScreen(props: { route: { params: unknown } }) {
      receivedParams = props.route.params;
  ...
  it('setParams() targets a route by key when given, not just the focused one', async () => {
    ...
    function HomeTrackingScreen(props: { route: { key: string; params: unknown } }) {
      homeKey = props.route.key;
      homeParams = props.route.params;
      return h('symbiote-text', {}, 'home');
    }
    const handleRef = ref<INavigatorHandle | null>(null);
    mountStack(handleRef, [
