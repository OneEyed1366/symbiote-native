...
// one) was ALSO missing — same class of gap, caught only once headerSearchBarOptions was actually
// exercised live.
//
// RNSScreenContentWrapper (the RNS_SCREEN_CONTENT_WRAPPER_VIEW_NAME wrapper stack.ts wraps every
// screen's content in — see its comment in core/constants.ts) hit a SIBLING gap a third time, but
// on the NATIVE side this time: this file alone was not enough. The native iOS registry
// (RCTThirdPartyComponentsProvider.mm, generated by codegen from THIS package's own
// package.json#codegenConfig.ios.componentProvider map, NOT from react-native-screens') had no
// entry for 'RNSScreenContentWrapper' either, so Fabric fell back to its "Unimplemented
// component" placeholder view regardless of this file's fix. Both registries are independently
// required for a never-before-used react-native-screens view: this file (JS ViewConfig, read by
// @symbiote-native/engine's registry.ts) and package.json's componentProvider map (native
// ComponentDescriptor, read by codegen at `pod install` time). Same class of miss as
// RNSSearchBar/RNSScreenStackHeaderSubview above, just on the other registry.
//
// RNSModalScreen was missing a FOURTH time, on both registries again — but this one wasn't a
// mount crash, it was a silent mispositioning bug. react-native-screens' own Screen.tsx picks
