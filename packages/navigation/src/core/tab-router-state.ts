...
// Mirrors CommonActions.setParams: a merge onto the existing object, never a replace — same
// fold navigator-state.ts's own setParams action performs.
function mergeParams(current: unknown, incoming: unknown): unknown {
  return isParamsRecord(current) && isParamsRecord(incoming)
    ? { ...current, ...incoming }
    : incoming;
}
