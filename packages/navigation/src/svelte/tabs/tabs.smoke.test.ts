...
function screenSource(testID: string): string {
  return `<script lang="ts">
   ...
   <symbiote-view {...{ testID: '${testID}', accessibilityLabel: [route.current.name, String(focused.current), JSON.stringify(route.current.params ?? null), route.current.key].join('|') }} />`;
}
...
async function mountEmptyTab(): Promise<ITabNavigatorHandle> {
  const app = await harness.compileSource(__dirname, 'tab-app-empty', emptyAppSource());
  const App = await loadComponent(app);
  let handle: unknown = null;
  mount(ROOT_TAG, App, {
    onReady: (value: unknown) => {
...
async function mountToggleTab(): Promise<{
  ...
  await harness.compileSource(dir, 'feed-fixture', screenSource('feed'));
  await harness.compileSource(dir, 'profile-fixture', screenSource('profile'));
  const app = await harness.compileSource(dir, 'tab-app-toggle', toggleAppSource());
  const App = await loadComponent(app);
  let handle: unknown = null;
  mount(ROOT_TAG, App, {
    onReady: (value: unknown) => {
