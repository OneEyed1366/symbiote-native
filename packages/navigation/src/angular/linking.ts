...
// passing `nav.handle`), the same top-level-prop shape @react-navigation keeps `linking` in on
// NavigationContainer rather than baking it into the router itself. Mirrors react/linking.ts's
// useLinkingIntegration; Angular needs none of its `configRef`/`handleRef` staleness workaround —
// this function runs its subscription ONCE per call (the natural `inject()` call-site convention,
// matching "call once per component instance"), and `navigatorHandle`'s methods already read
// live `this` state off the Stack component instance, so there is no stale-closure risk to guard
// against here.
