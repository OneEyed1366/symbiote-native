...
let receivedRouteKey: string | undefined;
...
class ParamsScreenComponent {
  ...
  ngOnChanges(): void {
    receivedParams = this.route.params;
    receivedRouteKey = this.route.key;
  }
}
...
describe('Angular Stack navigator', () => {
  ...
  it('setParams() targets a route by key when given, not just the focused one', async () => {
    receivedParams = undefined;
    receivedRouteKey = undefined;
    const handle = await mountParamsStack();
    // Home mounts first (ParamsScreenComponent, initialParams: {tab:'feed'}), capturing its key.
    const homeKey = receivedRouteKey;
    if (homeKey === undefined) throw new Error('home route key was never captured');
    handle.push('Details');
    await tick();
    handle.setParams({ tab: 'search' }, homeKey);
    await tick();
    expect(receivedParams).toEqual({ tab: 'search' });
  });
