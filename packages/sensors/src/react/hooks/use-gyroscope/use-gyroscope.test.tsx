// Co-located React-driven test (ADR 0025) for useGyroscope. Mocks the whole `core`
...
// already has its own coverage in packages/sensors/src/core/gyroscope.test.ts.
//
// No Negative group: the hook has no guard clause and nothing to reject — every prop shape
// the type system allows flows straight into Gyroscope.addListener/setUpdateInterval, so
// every scenario below is Positive. `Gyroscope.removeAllListeners` /
...
const results: Array<IGyroscopeMeasurement | null> = [];

function Probe(props: { updateIntervalMs?: number }): ReactElement {
  results.push(useGyroscope(props.updateIntervalMs));
