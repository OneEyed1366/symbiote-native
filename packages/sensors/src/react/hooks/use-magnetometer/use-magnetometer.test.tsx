// Co-located React-driven test (ADR 0025) for useMagnetometer. Mocks the whole `core`
...
// already has its own coverage in packages/sensors/src/core/magnetometer.test.ts.
//
// No Negative group: the hook has no guard clause and nothing to reject — every prop shape
// the type system allows flows straight into Magnetometer.addListener/setUpdateInterval, so
// every scenario below is Positive. `Magnetometer.removeAllListeners` /
...
const { addListener, removeAllListeners, setUpdateInterval, remove } = vi.hoisted(() => {
  const remove = vi.fn();
  return {
    addListener: vi.fn((_listener: (measurement: IMagnetometerMeasurement) => void) => ({
      remove,
    })),
...
vi.mock('../../../core', () => ({
  Magnetometer: { addListener, removeAllListeners, setUpdateInterval },
}));

const ROOT_TAG = 902;

const results: Array<IMagnetometerMeasurement | null> = [];

function Probe(props: { updateIntervalMs?: number }): ReactElement {
  results.push(useMagnetometer(props.updateIntervalMs));
