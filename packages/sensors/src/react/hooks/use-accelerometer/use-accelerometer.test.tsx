// Co-located React-driven test (ADR 0025) for useAccelerometer. Mocks the whole `core`
...
// already has its own coverage in packages/sensors/src/core/accelerometer.test.ts.
//
// No Negative group: the hook has no guard clause and nothing to reject — every prop shape
// the type system allows flows straight into Accelerometer.addListener/setUpdateInterval, so
// every scenario below is Positive. `Accelerometer.removeAllListeners` /
...
const { addListener, removeAllListeners, setUpdateInterval, remove } = vi.hoisted(() => {
  const remove = vi.fn();
  return {
    addListener: vi.fn((_listener: (measurement: IAccelerometerMeasurement) => void) => ({
      remove,
    })),
...
vi.mock('../../../core', () => ({
  Accelerometer: { addListener, removeAllListeners, setUpdateInterval },
}));

const ROOT_TAG = 901;

const results: Array<IAccelerometerMeasurement | null> = [];

function Probe(props: { updateIntervalMs?: number }): ReactElement {
  results.push(useAccelerometer(props.updateIntervalMs));
