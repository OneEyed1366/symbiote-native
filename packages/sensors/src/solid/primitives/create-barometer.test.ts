// Co-located Solid-driven test (ADR 0025) for createBarometer, the Solid twin of
// react/hooks/use-barometer, vue/composables/use-barometer and
// svelte/runes/use-barometer.svelte.ts. Same coverage as the Vue file — seed, listener-driven
...
// core/barometer.test.ts, and there is no native view here, so no ViewConfig fixture.
//
...
import { createBarometer } from './create-barometer';
...
const FIRST_READING: IBarometerMeasurement = {
  pressure: 1013.25,
  relativeAltitude: 12.3,
  timestamp: 1,
};
const SECOND_READING: IBarometerMeasurement = {
  pressure: 950,
...
function buildBarometer(
  ...
  const { value, dispose } = inRoot(() => createBarometer(updateIntervalMs));
  disposeRoot = dispose;
  return value;
}
...
describe('createBarometer (Solid)', () => {
  describe('Positive', () => {
    it('starts null before any measurement arrives', () => {
      ...
      const measurement = buildBarometer();
    ...
    it('updates the accessor when the native listener fires', () => {
      ...
      const seen: (IBarometerMeasurement | null)[] = [];
      const { value: measurement, dispose } = inRoot(() => {
        const reading = createBarometer();
        createEffect(() => {
          seen.push(reading());
        });
        return reading;
    ...
    it('replaces the previous measurement rather than merging with it', () => {
      ...
      // relativeAltitude behind after a real reading changes.
      const measurement = buildBarometer();
    ...
    it('does not touch the update interval when omitted', () => {
      ...
      // Barometer singleton already configured.
      buildBarometer();
