// Co-located Solid-driven test (ADR 0025) for createMagnetometerUncalibrated, the Solid twin of
// react/hooks/use-magnetometer-uncalibrated, vue/composables/use-magnetometer-uncalibrated and
// svelte/runes/use-magnetometer-uncalibrated.svelte.ts. Same coverage as the Vue file — seed, listener-driven
...
// core/magnetometer-uncalibrated.test.ts, and there is no native view here, so no ViewConfig fixture.
//
...
import { createMagnetometerUncalibrated } from './create-magnetometer-uncalibrated';
...
const FIRST_READING: IMagnetometerUncalibratedMeasurement = {
  x: 0.1,
  y: 0.2,
  z: 0.3,
  timestamp: 1,
};
const SECOND_READING: IMagnetometerUncalibratedMeasurement = {
  x: 9,
  y: 9,
  z: 9,
  timestamp: 2,
...
function buildMagnetometerUncalibrated(
  ...
  const { value, dispose } = inRoot(() =>
    createMagnetometerUncalibrated(updateIntervalMs),
  );
  disposeRoot = dispose;
  return value;
}
...
describe('createMagnetometerUncalibrated (Solid)', () => {
  describe('Positive', () => {
    it('starts null before any measurement arrives', () => {
      ...
      const measurement = buildMagnetometerUncalibrated();
    ...
    it('updates the accessor when the native listener fires', () => {
      ...
      const seen: (IMagnetometerUncalibratedMeasurement | null)[] = [];
      const { value: measurement, dispose } = inRoot(() => {
        const reading = createMagnetometerUncalibrated();
        createEffect(() => {
          seen.push(reading());
        });
        return reading;
    ...
    it('sets the update interval once at subscribe time when provided', () => {
      ...
      buildMagnetometerUncalibrated(50);
    ...
    it('sets the update interval for an explicit 0ms, not just a truthy value', () => {
      ...
      buildMagnetometerUncalibrated(0);
    ...
    it('does not touch the update interval when omitted', () => {
      ...
      // MagnetometerUncalibrated singleton already configured.
      buildMagnetometerUncalibrated();
    ...
    it('re-applies the update interval when a reactive accessor changes, without re-subscribing', () => {
      ...
      buildMagnetometerUncalibrated(intervalMs);
      expect(setUpdateIntervalMock).toHaveBeenCalledTimes(1);
