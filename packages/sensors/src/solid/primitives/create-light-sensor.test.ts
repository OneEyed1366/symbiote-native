// Co-located Solid-driven test (ADR 0025) for createLightSensor, the Solid twin of
// react/hooks/use-light-sensor, vue/composables/use-light-sensor and
// svelte/runes/use-light-sensor.svelte.ts. Same coverage as the Vue file — seed, listener-driven
...
// core/light-sensor.test.ts, and there is no native view here, so no ViewConfig fixture.
//
...
import { createLightSensor } from './create-light-sensor';
...
const FIRST_READING: ILightSensorMeasurement = {
  illuminance: 42,
  timestamp: 1,
};
const SECOND_READING: ILightSensorMeasurement = {
  illuminance: 900,
  timestamp: 2,
};
...
function buildLightSensor(
  ...
  const { value, dispose } = inRoot(() => createLightSensor(updateIntervalMs));
  disposeRoot = dispose;
  return value;
}
...
describe('createLightSensor (Solid)', () => {
  describe('Positive', () => {
    it('starts null before any measurement arrives', () => {
      ...
      const measurement = buildLightSensor();
    ...
    it('updates the accessor when the native listener fires', () => {
      ...
      const seen: (ILightSensorMeasurement | null)[] = [];
      const { value: measurement, dispose } = inRoot(() => {
        const reading = createLightSensor();
        createEffect(() => {
          seen.push(reading());
        });
        return reading;
    ...
    it('replaces the previous measurement rather than merging with it', () => {
      // why: each native event is a full snapshot, not a delta — merging would leave a stale
      // illuminance behind after a real reading changes.
      const measurement = buildLightSensor();
    ...
    it('does not touch the update interval when omitted', () => {
      ...
      // LightSensor singleton already configured.
      buildLightSensor();
