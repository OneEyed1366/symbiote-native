// Co-located Solid-driven test (ADR 0025) for createAccelerometer, the Solid twin of
// react/hooks/use-accelerometer, vue/composables/use-accelerometer and
// svelte/runes/use-accelerometer.svelte.ts. Same coverage as the Vue file — seed, listener-driven
...
// core/accelerometer.test.ts, and there is no native view here, so no ViewConfig fixture.
//
...
import { createAccelerometer } from './create-accelerometer';
...
const FIRST_READING: IAccelerometerMeasurement = {
  x: 0.1,
  y: 0.2,
  z: 0.3,
  timestamp: 1,
};
const SECOND_READING: IAccelerometerMeasurement = {
  x: 9,
  y: 9,
  z: 9,
  timestamp: 2,
...
function buildAccelerometer(
  ...
  const { value, dispose } = inRoot(() =>
    createAccelerometer(updateIntervalMs),
  );
  disposeRoot = dispose;
  return value;
}
...
describe('createAccelerometer (Solid)', () => {
  describe('Positive', () => {
    it('starts null before any measurement arrives', () => {
      ...
      const measurement = buildAccelerometer();
    ...
    it('updates the accessor when the native listener fires', () => {
      ...
      const seen: (IAccelerometerMeasurement | null)[] = [];
      const { value: measurement, dispose } = inRoot(() => {
        const reading = createAccelerometer();
        createEffect(() => {
          seen.push(reading());
        });
        return reading;
    ...
    it('does not touch the update interval when omitted', () => {
      ...
      // Accelerometer singleton already configured.
      buildAccelerometer();
