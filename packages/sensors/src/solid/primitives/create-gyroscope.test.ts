// Co-located Solid-driven test (ADR 0025) for createGyroscope, the Solid twin of
// react/hooks/use-gyroscope, vue/composables/use-gyroscope and
// svelte/runes/use-gyroscope.svelte.ts. Same coverage as the Vue file — seed, listener-driven
...
// core/gyroscope.test.ts, and there is no native view here, so no ViewConfig fixture.
//
...
import { createGyroscope } from './create-gyroscope';
...
const FIRST_READING: IGyroscopeMeasurement = {
  x: 0.1,
  y: 0.2,
  z: 0.3,
  timestamp: 1,
};
const SECOND_READING: IGyroscopeMeasurement = {
  x: 9,
  y: 9,
  z: 9,
  timestamp: 2,
...
function buildGyroscope(
  ...
  const { value, dispose } = inRoot(() => createGyroscope(updateIntervalMs));
  disposeRoot = dispose;
  return value;
}
...
describe('createGyroscope (Solid)', () => {
  describe('Positive', () => {
    it('starts null before any measurement arrives', () => {
      ...
      const measurement = buildGyroscope();
    ...
    it('updates the accessor when the native listener fires', () => {
      ...
      const seen: (IGyroscopeMeasurement | null)[] = [];
      const { value: measurement, dispose } = inRoot(() => {
        const reading = createGyroscope();
        createEffect(() => {
          seen.push(reading());
        });
        return reading;
    ...
    it('does not touch the update interval when omitted', () => {
      ...
      // Gyroscope singleton already configured.
      buildGyroscope();
