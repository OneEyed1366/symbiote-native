...
// shape the type system allows flows straight into
// MagnetometerUncalibrated.addListener/setUpdateInterval, so every scenario below is Positive.
// `MagnetometerUncalibrated.removeAllListeners` and the rest of DeviceSensor's async API are
// N/A here — the composable never calls them; they're core-level surface covered by
// magnetometer-uncalibrated.test.ts.
...
describe('useMagnetometerUncalibrated (Vue)', () => {
  describe('Positive', () => {
    ...
    it('sets the update interval for an explicit 0ms, not just a truthy value', () => {
      ...
      mountMagnetometerUncalibrated(0);
    ...
    it('does not touch the update interval when omitted', () => {
      ...
      // shared MagnetometerUncalibrated singleton already configured.
      mountMagnetometerUncalibrated();
