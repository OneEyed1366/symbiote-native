...
@Component({
  selector: 'symbiote-accelerometer-zero-interval-host',
  standalone: true,
  template: '',
})
class AccelerometerZeroIntervalHost {
  readonly measurement = inject(AccelerometerService).connect(0);
  ...
  describe('Positive (expected to succeed without an error)', () => {
    ...
    describe('the optional updateIntervalMs param', () => {
      ...
      it('calls setUpdateInterval with 0 rather than skipping it (0 is a valid interval, not "no interval")', async () => {
        ...
        mount(ROOT_TAG, AccelerometerZeroIntervalHost);
        await tick();
