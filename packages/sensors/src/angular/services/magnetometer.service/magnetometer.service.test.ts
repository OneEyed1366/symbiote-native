...
@Component({
  selector: 'symbiote-magnetometer-zero-interval-host',
  standalone: true,
  template: '',
})
class MagnetometerZeroIntervalHost {
  readonly measurement = inject(MagnetometerService).connect(0);
  ...
  describe('Positive (expected to succeed without an error)', () => {
    ...
    describe('the optional updateIntervalMs param', () => {
      ...
      it('calls setUpdateInterval with 0 rather than skipping it (0 is a valid interval, not "no interval")', async () => {
        ...
        mount(ROOT_TAG, MagnetometerZeroIntervalHost);
        await tick();
