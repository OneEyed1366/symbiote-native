...
@Component({
  selector: 'symbiote-light-sensor-zero-interval-host',
  standalone: true,
  template: '',
})
class LightSensorZeroIntervalHost {
  readonly measurement = inject(LightSensorService).connect(0);
...
describe('LightSensorService.connect', () => {
  describe('Positive (expected to succeed without an error)', () => {
    it('reports null before any measurement event fires', async () => {
      // why: the template reads `measurement()?.illuminance` — the signal must start at a
    ...
    describe('the optional updateIntervalMs param', () => {
      ...
      it('calls setUpdateInterval with 0 rather than skipping it (0 is a valid interval, not "no interval")', async () => {
        ...
        mount(ROOT_TAG, LightSensorZeroIntervalHost);
        await tick();
