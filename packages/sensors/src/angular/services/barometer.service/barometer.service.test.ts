// Co-located Angular-driven test (ADR 0025) for BarometerService. Mounts a real host
...
import { BarometerService } from './index';
...
@Component({
  selector: 'symbiote-barometer-interval-host',
  standalone: true,
  template: '',
})
class BarometerIntervalHost {
  readonly measurement = inject(BarometerService).connect(500);
