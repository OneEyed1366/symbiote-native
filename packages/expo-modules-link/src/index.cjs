...
// Unlike MODULES-MAP, no `getServices()` stub exists in a hand-written ExpoModulesProvider
// (AppContext defaults it to `emptyList()`) - the whole method is generated content, anchored
// right after the class opens.
const MODULES_PROVIDER_CLASS_ANCHOR = ': ModulesProvider {';
...
function patchBuildGradle(appRoot, entries) {
  ...
  const primaryProjectNames = entries
  ...
  // A `services` entry can back onto a different native package than the one declaring it (e.g.
  // task-manager's native-link.json lists expo-constants' ConstantsService) - that package's
  // Gradle subproject needs including too, same as any primary gradleProjectName.
  const serviceProjectNames = androidServicesOf(entries).map((service) => service.gradleProjectName);
  const deps = [...new Set([...primaryProjectNames, ...serviceProjectNames])]
    .sort()
...
function androidServicesOf(entries) {
  return entries.flatMap((entry) =>
    entry.manifest.android && Array.isArray(entry.manifest.android.services) ? entry.manifest.android.services : [],
  );
}
...
function patchMainApplication(appRoot, entries) {
  ...
  const services = androidServicesOf(entries);
  const imports = [
    ...new Set([
      ...modules.map((module) => `import ${module.importPath}`),
      ...services.map((service) => `import ${service.importPath}`),
      // Only getServices()'s return type needs this import, but it's unconditional because the
      // method itself is always regenerated below (see the SERVICES region), signature and all.
  ...
  const serviceEntries = [...new Set(services.map((service) => `  ${service.className}::class.java,`))].sort();
  ...
  if (withModules === null) return;

  // Unlike MODULES-MAP, there's no pre-existing `getServices()` stub to anchor entries into, so
  // the whole method (header + entries + closing paren) is the region body, generated right after
  // the class opens (before getModulesMap()).
  const next = writeRegion({
    filePath,
    content: withModules,
    name: 'SERVICES',
    indent: '  ',
    bodyLines: ['override fun getServices(): List<Class<out Service>> = listOf(', ...serviceEntries, ')'],
    findAnchorIndex: (lines) => lines.findIndex((line) => line.includes(MODULES_PROVIDER_CLASS_ANCHOR)),
    anchorLabel: 'no `ExpoModulesProvider : ModulesProvider` class declaration to anchor below',
  });
...
// Same tag-boundary scan as findTag, but returns every match: <activity> can appear more than
// once, and only the one named MainActivity matters here.
  ...
  while (match) {
    ...
    for (let i = match.index; i < content.length; i += 1) {
      ...
      if (char === '"' || char === "'") quote = char;
        ...
        tags.push({ start: match.index, end: i });
        re.lastIndex = i + 1;
        break;
      }
    }
...
// Sibling of patchAndroidManifest, but MERGES into MainActivity's existing android:configChanges
// value instead of skipping when the attribute is present - every RN template already ships it
// with a base value, so "already present" can't be the idempotency signal here.

// Why this shape exists: packages/localization's plugin defaults allowDynamicLocaleChangesAndroid
// to true, appending locale|layoutDirection so onConfigurationChanged fires instead of the
// Activity restarting on locale change (protects useLocales() Android reactivity).
function patchMainActivityConfigChanges(appRoot, entries) {
  const wanted = new Set();
  for (const entry of entries) {
    const values = entry.manifest.android && entry.manifest.android.mainActivityConfigChanges;
    if (!values) continue;
    for (const value of values) wanted.add(value);
  }
  if (wanted.size === 0) return;

  const manifestPath = path.join(appRoot, 'android', 'app', 'src', 'main', 'AndroidManifest.xml');
  if (!fs.existsSync(manifestPath)) {
    dlog('AndroidManifest.xml not found, skipping MainActivity configChanges');
    return;
  }
  ...
  const existingValue = readXmlAttribute(tagText, 'android:configChanges');
  const existingTokens = existingValue
    ? existingValue.split('|').map((token) => token.trim()).filter(Boolean)
    : [];
  const existingSet = new Set(existingTokens);
  const missing = [...wanted].filter((token) => !existingSet.has(token)).sort();
  if (missing.length === 0) return;

  const merged = [...existingTokens, ...missing].join('|');
  let next;
  if (existingValue !== null) {
    const valueMatch = /android:configChanges\s*=\s*"/.exec(tagText);
    ...
    next = content.slice(0, tag.start) + nextTagText + content.slice(tag.end);
  } else {
    const lastNewline = tagText.lastIndexOf('\n');
    const separator = lastNewline === -1 ? ' ' : `\n${/^[ \t]*/.exec(tagText.slice(lastNewline + 1))[0]}`;
    const selfClosing = content[tag.end - 1] === '/';
    const insertAt = selfClosing ? tag.end - 1 : tag.end;
    // A self-closing tag's own trailing space (before the `/`) would otherwise double up with
    // `separator`'s leading space; put exactly one back between the new attribute and `/>`.
    const before = content.slice(0, insertAt).replace(/[ \t]+$/, '');
    const trailer = selfClosing ? ' ' : '';
    next = `${before}${separator}android:configChanges="${merged}"${trailer}${content.slice(insertAt)}`;
  }
