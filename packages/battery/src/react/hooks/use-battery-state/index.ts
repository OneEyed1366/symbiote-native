...
export function useBatteryState(): BatteryState {
  const [batteryState, setBatteryState] = useState<BatteryState>(BatteryState.UNKNOWN);

  useEffect(() => {
    getBatteryStateAsync().then(setBatteryState);
    const subscription = addBatteryStateListener(event => setBatteryState(event.batteryState));
