...
// it is the plugin PLUS the isCustomElement option that makes the `view`/`text`/… intrinsic tags
// compile to their lowered intrinsic element instead of to a Vue component instance each. Either
// half alone is broken — see @symbiote-native/vue/babel-jsx, which is why they arrive as one
// require() rather than two lines here. @vue/babel-plugin-jsx is a dependency of the adapter now,
// so a fresh install of a published @symbiote-native/vue brings it along; this app's own entry for
