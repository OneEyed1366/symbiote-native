// SectionList — sections flattened into one virtualized stream. Each section
// contributes a header row, its item rows, then a footer row (RN's
...
// renderSectionFooter paints footers.
...
export function SectionList<ItemT>(props: SectionListProps<ItemT>): ReactElement {
  ...
  dlog(`SectionList: ${sections.length} sections flattened to ${entries.length} entries`)
