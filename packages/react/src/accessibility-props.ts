...
// forwards any non-function / non-style prop straight to Fabric (shared's
...
export interface AccessibilityStateValue {
  disabled?: boolean
  selected?: boolean
...
export interface AccessibilityProps {
  ...
  onAccessibilityAction?: (event: SymbioteEvent) => void
  // iOS-only
  onAccessibilityTap?: (event: SymbioteEvent) => void
  // iOS-only
  onMagicTap?: (event: SymbioteEvent) => void
  // iOS-only
  onAccessibilityEscape?: (event: SymbioteEvent) => void
}
...
const ROLE_TO_ACCESSIBILITY_ROLE: Readonly<Record<string, AccessibilityRole>> = {
  alert: 'alert',
  button: 'button',
  checkbox: 'checkbox',
  combobox: 'combobox',
...
function accessibilityRoleFromRole(role: Role): AccessibilityRole {
  return ROLE_TO_ACCESSIBILITY_ROLE[role] ?? role
}

const ARIA_KEYS: ReadonlyArray<keyof AriaProps> = [
  'role',
  'aria-label',
  'aria-labelledby',
  'aria-live',
...
function hasAnyAriaKey(props: AriaProps): boolean {
...
export function resolveAccessibilityProps<T extends AccessibilityProps & AriaProps>(
  props: T,
): T {
