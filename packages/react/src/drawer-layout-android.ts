// DrawerLayoutAndroid — the platform DrawerLayout (Android only). AndroidDrawerLayout
// is an ordinary Fabric host node: it lives in the SAME childSet and commits through
// the SAME completeRoot as the rest of the tree, exactly like Modal/Switch. There is
// no per-library glue — shared derives the view's events/processors from its
// ViewConfig, so we render the RAW Fabric name `AndroidDrawerLayout` (the derive-by-
// default path: any non-`symbiote-*` createElement type flows through untouched).
//
// Behavior mirrors RN's DrawerLayoutAndroid.android.js: the navigation view (from
// `renderNavigationView()`) and the main content are wrapped in two collapsable=false
// container Views, and — critically — the child order RN commits is
// [mainSubview, drawerSubview]: the content wrapper FIRST, the navigation wrapper
// SECOND (android render: {childrenWrapper}{drawerViewWrapper}). The navigation
// wrapper is absolutely positioned, drawerWidth-wide, and gated by pointerEvents so
// it stays untouchable until opened. `openDrawer`/`closeDrawer` are imperative,
// reached through a ref, and dispatched as the `openDrawer`/`closeDrawer` view
// commands against the host node — mirroring Switch's dispatchViewCommand path
// (Commands.openDrawer/closeDrawer in AndroidDrawerLayoutNativeComponent).
//
// iOS / no-native-view degrade: AndroidDrawerLayout does not exist off Android, so
// like RN's DrawerLayoutAndroidFallback (.ios.js) we render the children in a plain
// container View and leave the navigation view unmounted. The imperative open/close
// become no-ops there (RN throws; we stay silent under DEBUG) — there is no drawer
// to drive.
...
// The imperative API a host ref hands back — RN's DrawerLayoutAndroidMethods, pared
// to the two drawer commands (measure/setNativeProps already ride the host instance).
...
// RN's drawerState int -> string mapping (android: DRAWER_STATES indexed by the
// native drawerState). 0=Idle, 1=Dragging, 2=Settling.
...
// there is no node yet / off Android (RN's fallback throws; we degrade quietly since
// there is no drawer to drive).
...
>((props, ref) => {
  ...
  // _onDrawerOpen/_onDrawerClose setState). Closed -> 'none' so the off-screen
  // drawer never intercepts touches; open -> 'auto'.
  ...
  // iOS / no-native-view degrade: AndroidDrawerLayout is Android-only, so off Android
  // we render the content alone in a plain container and drop the navigation view —
  // RN's DrawerLayoutAndroidFallback shape. No host node means the imperative commands
  // are silent no-ops (dispatchDrawerCommand guards on node===null).
  if (Platform.OS !== 'android') {
    dlog('DrawerLayoutAndroid degrade: non-Android host, rendering content only')
