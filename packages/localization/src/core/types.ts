// Hand-ported verbatim from .vendors/expo/packages/expo-localization/src/Localization.types.ts
// (sdk-57) — plain data shapes, no expo-modules-core dependency at all. Every JSDoc comment is
// kept as-is: they document real, per-platform behavior differences, not restated obviousness.
...
export enum CalendarIdentifier {
  ...
  ETHIOPIC = 'ethiopic',
  /** Gregorian calendar (also referred to upstream as "gregorian" — dropped as a separate
   * member here since this repo's lint bans duplicate enum values). */
  GREGORY = 'gregory',
