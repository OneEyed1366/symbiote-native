...
import { dlog, getActiveSurfaces, isDebug, registerPostCommit } from '@symbiote-native/engine';
import { DEVTOOLS_PLUGIN_ID, type IDevtoolsEvents, type ISerializedSurface } from './src/protocol';
...
async function setupDevtoolsInspector(): Promise<void> {
  ...
  const client = await getRozeniteDevToolsClient<IDevtoolsEvents>(DEVTOOLS_PLUGIN_ID);
  ...
  const sendSnapshotIfSubscribed = () => {
    ...
    dlog(() => `devtools inspector: sending snapshot, ${surfaces.length} surface(s)`);
  ...
  client.onMessage('subscribe', () => {
    ...
    if (!hookRegistered) {
      hookRegistered = true;
      registerPostCommit(sendSnapshotIfSubscribed);
    }
