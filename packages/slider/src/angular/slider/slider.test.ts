...
//
// No custom-StepMarker coverage here: shared.ts's module doc comment records it as a deliberate,
// scoped-out gap (no `#stepMarker`-equivalent content projection wired yet) — N/A, not
// characterization, per that comment's own rationale. React's and Vue's slider.test cover the
// custom-marker overlay since both adapters implement it.
...
// SliderBase's `descriptor` getter has no throwing path — it's pure prop folding + descriptor
// assembly over caller @Input values. A single Positive group; no Negative one.
...
describe('Angular Slider anchor class= resolution', () => {
  describe('Positive', () => {
    ...
    it('resolves a class= on the Slider use site onto the real committed centering wrapper View', async () => {
      // why: two anchor levels sit between the use-site element and the real Fabric node
      // (Slider's own host anchor, then DescriptorOutlet's) — a naive merge could easily stamp
      // the class onto the wrong one and silently drop the style on a real device.
