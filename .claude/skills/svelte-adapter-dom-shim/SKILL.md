...
description: "SUPERSEDED (2026-08-16) - the DOM-shim strategy this skill documents has been fully replaced by Svelte's official custom-renderer API. Read `svelte-adapter-custom-renderer` instead for all current adapters/svelte/** work. This file is kept only as historical/fallback reference in case a future Svelte release breaks the official API and the project has to revert to a shim (unlikely - sveltejs/svelte PR #18042 is the API this project now runs on, via a git-commit pin). Original description, for archive purposes: Symbiote Svelte adapter - the DOM-shim strategy and its exact hand-maintained surface. Svelte's OFFICIAL custom-renderer API (sveltejs/svelte#18042, `createRenderer` from `svelte/renderer`) was an UNMERGED PR at the time this was written, so the adapter patched globalThis DOM classes so stock compiled Svelte output ran unchanged - a deliberate, accepted coupling to Svelte PRIVATE internals. Holds: the measured mandatory DOM surface with file:line (init_operations' prototypes/descriptors/private fields; the 6 document factories; from_tree's build-once-then-clone; the universal `anchor.before()` mount path; createDocumentFragment in each/if/boundary); the CUSTOM-ELEMENT codegen path every hyphenated symbiote-* tag took - importNode replaced cloneNode, and set_custom_element_data stringified scalars and hard-excluded `style`, resolved by passing ONE object bag prop unpacked into routeProp; the dead-if-forbidden surface (svelte:head, element bind:, hydration, svelte:element, autofocus); the DELEGATED_EVENTS dodge; the ISymbioteEvent extensibility requirement; the measured React-Native 0.86 global collisions; and the lazy-engine-node design. Trigger on: 'svelte dom shim', 'patchGlobals', 'old svelte adapter', or historical/regression research - NOT for any current adapters/svelte/** task, which is 'svelte-adapter-custom-renderer'."
---
