...
- `.examples/*` — native wiring (Podfile, Android Gradle autolinking, `MainApplication.kt`/
  `AppDelegate` bootstrap, Info.plist entries) is its own later step. This holds even if
  `.examples/react` is entirely absent from the current worktree — it's a gitignored,
  not-always-checked-out dev harness (see `symbiote-dev-examples`), its absence is not a signal
  to skip wiring, just a signal that wiring can't happen in *this* worktree right now.
