...
§fix_esm_extensions_missing_pkg := {
  mechanism: "scripts/fix-esm-extensions.mjs rewrites extension-less relative imports
              tsc --build emits (Node ESM loader rejects them outside a bundler) —
              ONLY for build/ dirs passed as CLI args in the root `fix-esm-extensions` script;
              not inferred from files/publishConfig, a flat easy-to-forget list",
  ...
  rule: "adding a new publishable package with its own build/ → add it to this
         script's argument list in the SAME change"
}
```
...
§pnpm_publish_2fa_hang := {
  ...
  fix: "split the job — pnpm pack (tmpdir) then `npm publish <tarball>
        --access public`; this is what scripts/trust-publishers.mjs does",
