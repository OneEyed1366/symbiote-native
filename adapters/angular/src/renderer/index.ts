...
export class SymbioteRenderer implements Renderer2 {
  ...
  setProperty(el: IHostElement, name: string, value: unknown): void {
    ...
    noteAngularWrite(name);
    const aliased = aliasedPropName(name);
    const resolved = value === undefined ? textDefaultFor(el, aliased) : value;
    ...
    if (aliased === VALUE_CHANGE_PROP) {
      if (
        isReadBackListener(resolved) &&
        !bridgedValueChangeHandlers.has(resolved)
      ) {
        explicitValueChangeHandlers.set(el, resolved);
      } else {
        explicitValueChangeHandlers.delete(el);
      }
      const current = el.props[VALUE_CHANGE_PROP];
      const bridgeActive =
  ...
  listen(
    ...
    if (eventName === VALUE_CHANGE_EVENT) {
      ...
      const currentOnValueChange = target.props[VALUE_CHANGE_PROP];
      if (
