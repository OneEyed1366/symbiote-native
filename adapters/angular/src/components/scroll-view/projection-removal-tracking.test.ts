...
const rows: IRow[] = Array.from({ length: ITEM_COUNT }, (_unused, index) => ({
  id: index,
}));
const stickyIndices = rows
  .filter(row => row.id % SECTION_STRIDE === 0)
  .map(row => row.id);
...
describe('ScrollView projection tracks removals of non-wrapped projected children', () => {
  it('keeps sticky wrapping on the current section header after a long scroll', async () => {
    ...
    for (let offset = 0; offset <= 3100; offset += 100) {
      ...
      if (offset % 500 === 0)
        console.log(
          'AT',
          offset,
          JSON.stringify(renderedRowIds(fabric.committed)),
        );
    }
    ...
    const forcedSticky =
      Math.floor(windowStart / SECTION_STRIDE) * SECTION_STRIDE;
    console.log('RENDERED', JSON.stringify(rendered), 'forced', forcedSticky);
    const stragglers = rendered.filter(id => id < forcedSticky);
    expect(
      stragglers,
      'no already-recycled cell is resurrected into the content',
    ).toEqual([]);
    expect(
      rendered.includes(forcedSticky),
      'the current section header stays mounted',
    ).toBe(true);
  });
});
