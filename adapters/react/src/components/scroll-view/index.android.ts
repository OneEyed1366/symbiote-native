...
export const ScrollView = forwardRef<IScrollViewHandle, IScrollViewProps>(
  (props, forwardedRef) => {
    ...
    useImperativeHandle(
      forwardedRef,
      () => buildScrollViewHandle(() => ref.current),
      [],
    );
    ...
    dlog(
      'ScrollView.ANDROID refreshControl=' +
    ...
    if (refreshControl === undefined) {
      ...
      return createElement(
        scrollViewIntrinsic,
        { ...outerProps, style: scrollStyle, ref },
        content,
      );
    }
    ...
    const { className: _classNameAppliedViaSplit, ...innerOuterProps } =
      outerProps;
