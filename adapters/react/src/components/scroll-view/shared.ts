...
export function usePreparedScrollView(rawProps: IScrollViewProps): IPreparedScrollView {
  ...
  const stickyHeaderRefsRef = useRef<Map<number, IStickyHeaderHandle> | null>(null);
  if (stickyHeaderRefsRef.current === null) stickyHeaderRefsRef.current = new Map();
  const stickyHeaderRefs = stickyHeaderRefsRef.current;

  const onHeaderRef = (index: number, handle: IStickyHeaderHandle | null): void => {
  ...
  if (pendingHeaderYsRef.current === null) pendingHeaderYsRef.current = new Map();
