// `IRefreshControlProps`'s canonical home. `RefreshControl` is a bare `<refresh-control>` TAG, so
// this type has no component to be re-exported from. `onRefresh` rides the `p={{}}` bag as an
// idiomatic Svelte 5 callback prop, unlike Vue which re-wires it through a typed `refresh` emit.
