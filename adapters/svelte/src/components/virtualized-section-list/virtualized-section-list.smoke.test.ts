...
const ANIMATED_VIEW_OUT = join(MODULES_ANIMATED_DIR, '.section-smoke-compiled-animated-view.mjs');
const REFRESH_CONTROL_OUT = join(COMPONENTS_DIR, '.section-smoke-compiled-refresh-control.mjs');
...
const SECTION_LIST_OUT = join(__dirname, '.section-smoke-compiled-virtualized-section-list.mjs');
...
const WRAPPER_ROOT_OUT = join(__dirname, '.section-smoke-compiled-wrapper-root.mjs');
...
function compileSectionListTree(): void {
  ...
  const stickyHeader = compile(
    ...
    "from '../../modules/animated/.section-smoke-compiled-animated-view.mjs'",
  );
  writeFileSync(STICKY_HEADER_OUT, stickyHeader);

  const list = compile(
    readFileSync(join(COMPONENTS_DIR, 'virtualized-list', 'index.svelte'), 'utf8'),
  ...
  const sectionList = compile(readFileSync(join(__dirname, 'index.svelte'), 'utf8'), {
    ...COMPILE_OPTIONS,
...
async function loadRoot(source: string, filename: string, outPath: string): Promise<Component> {
...
describe('VirtualizedSectionList getItemLayout (real compiled index.svelte)', () => {
  describe('Positive', () => {
    ...
    it('relays getItemLayout through SectionList to the same sections argument', async () => {
      ...
      expect(seen.length, 'getItemLayout survived the SectionList relay').toBeGreaterThan(0);
      for (const data of seen) {
        expect(data, 'getItemLayout receives the sections array by identity').toBe(SECTIONS);
      }
    });
