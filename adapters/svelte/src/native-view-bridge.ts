...
// registered for this repo's tests — see svelte-adapter-custom-renderer skill, deliberately: unit
...
// descriptor-to-svelte.ts, renderer.ts), confirmed by grepping each for a `.svelte` import.
//
...
//
// `mountDescriptorChildren`/`createDescriptorChildrenSync`: the generic Descriptor -> engine-tree
// bridge, the Svelte twin of Vue's `descriptorToVue` / React's `descriptorToReact` (both exported
// from their own adapter's main barrel — Svelte's bridge lives here instead, precisely because
// its own home module is `.svelte`-free but the main barrel that would otherwise re-export it is
// not). Needed by a third-party-view package to mount a Descriptor whose `type` is a raw,
// non-`symbiote-`-prefixed Fabric name (e.g. 'RNCSlider') that cannot be written as a literal
// Svelte template tag — see packages/slider/src/svelte/slider/index.svelte's header for the full
// reasoning.
...
export type { ISymbioteNode } from '@symbiote-native/engine';
