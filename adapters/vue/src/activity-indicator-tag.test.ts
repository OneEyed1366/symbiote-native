// `activity-indicator` as a TAG, measured through Vue's own renderer. RN's ActivityIndicator is a
// centering `<View>` wrapped around a native spinner (ActivityIndicator.js:112) and takes no
...
function mountTag(props: Record<string, unknown>): Promise<void> {
  mount(
    ROOT_TAG,
    defineComponent({ setup: () => () => h('activity-indicator', props) }),
  );
  return settle();
}
...
describe('Vue: `activity-indicator` as a tag', () => {
  ...
  it('routes an app prop to the spinner and keeps the style on the host', async () => {
    await mountTag({ nativeID: 'ind', testID: 'spin', style: { margin: 4 } });
