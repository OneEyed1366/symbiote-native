...
describe('touchable-native-feedback as a tag', () => {
  ...
  // why: the count above is satisfied by an unregistered tag too — an anchor commits nothing on
  // its own. This is the arm that fails when the registration is missing.

  // `onLayout` clones as a real LISTENER (behavior's `FORWARDED_LISTENERS`), a Fabric
  // BOOLEAN-GATED event that shows up in the payload as `true` — still JS, unlike the clone
  // itself, which is `foldCloneOntoChild`'s in C++ now.
