...
bootstrap and demos every `expo-modules-core`-based wrapper package this repo ships (all 22 —
Application through Web Browser), one screen per package, reached from a `Menu` screen.
`examples/vue-sfc` itself stays a pure SymbioteNative canary — core primitives + navigation —
with zero expo-modules-core dependency.
...
App.vue                    the native stack navigator over the Menu + 22 wrapper-package demo screens
screens/                   MenuScreen plus one <Name>Screen.vue per wrapper package
...
Every screen is an ordinary Vue SFC. Metro has no Vue plugin (unplugin-vue ships
...
AppDelegate factory, `MainApplication.kt`'s generated module-provider registrations) that
`examples/vue-sfc` does not — that bootstrap exists solely to make expo-modules-core
autolinking work for the 22 wrapper-package demos.
