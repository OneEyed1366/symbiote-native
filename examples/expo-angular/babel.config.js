...
module.exports = {
  ...
  plugins: [
    ...
    // `tsconfig.angular.base.json` compiles at ES2022 with `useDefineForClassFields: false`, which
    // makes TypeScript lower a static property initializer into `static { this.ɵfac = … }`; RN's
    // preset carries the other class features but not this one, and
    // `@babel/helper-create-class-features-plugin` refuses a class that mixes them. It surfaces only
    // in a RELEASE bundle, where that transform runs. Same line, same reason, in `examples/angular`.
