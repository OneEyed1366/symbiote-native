...
// launchApp() must complete the New-Arch startup sync
...
describe('symbiote attach probe', () => {
  beforeAll(async () => {
    // The canary runs a perpetual native-driven Animated.loop heartbeat (App.tsx, offloaded to the
    // native driver), so the app never reports idle. Detox cannot sync against an infinite animation (true
