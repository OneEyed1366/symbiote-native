...
export class ProbeScreen {
  ...
  readonly idleTitle = computed(() =>
    this.runningMode() === SAMPLE_MODE.Idle
      ? 'Sampling, hands off...'
      : 'Sample 3 s idle',
  );

  readonly scrollTitle = computed(() =>
    this.runningMode() === SAMPLE_MODE.Scroll
      ? 'Sampling, keep dragging'
      : 'Sample 3 s scrolling',
  );
  ...
  readonly instructions = computed(() => {
    if (this.runningMode() === SAMPLE_MODE.Idle)
      return 'Do not touch the screen until it stops.';
    if (this.runningMode() === SAMPLE_MODE.Scroll)
      return 'Drag the list below for the whole window.';
  ...
  readonly verdicts = computed<readonly string[]>(() => {
    ...
    if (mode === SAMPLE_MODE.Idle) {
      ...
      if (commit.commits > 0) {
        found.push(
          `The engine committed ${commit.commits} times at rest, so the writes reach Fabric.`,
        );
      }
      if (found.length === 0) {
        found.push(
          'Clean at rest. The cost is inside a pass, not the number of passes - sample while scrolling next.',
        );
      }
      return found;
    }
    ...
    if (angular.scrollTicks === 0) {
      return [
        'No scroll events arrived. Drag the list below while the sample runs.',
      ];
    }
    ...
    if (found.length === 0) {
      found.push(
        'One pass per frame, cells reused. The cost is inside the pass - read the walk ms and the write count.',
      );
    }
    return found;
  });
