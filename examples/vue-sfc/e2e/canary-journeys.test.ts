...
// Each journey drives a real native interaction AND asserts the observable outcome of ONE component,
// so a component that mounts but MIS-behaves — e.g. a native-driver Animated view that renders
// frozen — fails HERE, where a bare toBeVisible would pass.
//
// Sync is OFF from the first launch: the canary runs a perpetual native Animated.loop heartbeat
// so the app never reports idle and launchApp would hang waiting for it; detoxEnableSynchronization:0
// via launchArgs is the same gate the attach probe uses.
...
// on Android support varies by Detox version.
