<!--
  ...
  The input is UNCONDITIONAL. It used to sit behind a row-shape toggle so one text-input could be
  priced as a delta; that number has been taken, and a second arm only splits every later
  measurement in two. `value` rather than `defaultValue` on purpose — a CONTROLLED input runs the
  behavior's afterCommit handshake on every commit, which is the work tier-2 moved onto the node.
