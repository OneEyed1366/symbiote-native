<script lang="ts">
  ...
  import { View, Text, PlatformColor, DynamicColorIOS, useColorScheme } from '@symbiote-native/svelte';
...
<View class="section-nested"><Text class="section-label">{`PlatformColor · semantic + DynamicColorIOS (${scheme.current ?? 'unknown'})`}</Text><View class="row"><View class="color-tile" style={{ backgroundColor: PlatformColor('systemBlue') }}><Text class="tile-label">systemBlue</Text></View><View
      class="color-tile-bordered"
      style={{
        backgroundColor: DynamicColorIOS({ light: '#dbeafe', dark: '#13243a' }),
        borderColor: PlatformColor('separator'),
      }}
    ><Text class="bold-label" style={{ color: PlatformColor('label') }}>dynamic</Text></View></View></View>
