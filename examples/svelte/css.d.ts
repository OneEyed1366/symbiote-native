...
// Generic (non-literal) fallback for a `.module.css` import — used by `svelte-check` outside a
// file that already has its own generated `Card.module.css.d.ts` (see `css-dts`; a real per-file
// `.d.ts` takes priority over this wildcard once generated). The
...
// standalone `tsc`/`svelte-check`/CI run, which is why both mechanisms exist side by side.
