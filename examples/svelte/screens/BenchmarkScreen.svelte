...
<SafeAreaView class="screen">
  ...
  >
    ...
    {#if hasSuiteResults}
      ...
      {/each}
    {:else}
      <Text testID="bench-suite-empty" class="note-text">No suite run yet.</Text>
    {/if}
    ...
    >
      {#snippet sectionHeader({ section })}
        <Text class="section-header" style={{ height: SECTION_LIST_HEADER_HEIGHT }}>
          {section.title}
        </Text>
      {/snippet}
      {#snippet item({ item })}
    ...
    {#if history.length === 0}
      <Text class="note-text">Run an operation above to record a measurement.</Text>
    {:else}
