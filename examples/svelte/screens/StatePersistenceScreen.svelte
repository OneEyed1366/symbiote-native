<script lang="ts">
  ...
  import { useNavigation, useNavigationState } from '@symbiote-native/navigation/svelte';
  ...
  const navigatorState = useNavigationState<INavigatorState>(currentState => currentState);
  ...
  function onRestore(): void {
    ...
    if (!('reset' in handle)) {
      restoreError = 'this screen is not mounted under a Stack — reset() is unavailable';
      return;
    }
    try {
  ...
  ><View class="section"
    ...
    ><View class="hero-card"
      ...
      ><View class="hero-copy"
        ...
        ><Text class="hero-body">The Stack's own state serialized out and deserialized back in — restoring exactly where you left off.</Text
      ></View
    ></View
    ><Text class="info-text">{`current stack depth: ${navigatorState.current.routes.length}`}</Text
    ><ActionButton
    ...
    /><ActionButton
      ...
      onPress={onRestore}
      color={LINE_COLOR.routing}
    />{#if restoreError !== undefined}<Text class="info-text">{`error: ${restoreError}`}</Text
    >{/if}<View class="box-list160"
      ><Text testID="persist-snapshot" class="list-row-text">{snapshot ?? 'tap Serialize to capture the current route stack as JSON'}</Text></View
    ></View
  ></SafeAreaView
>
