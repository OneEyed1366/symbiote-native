  ...
  ><View class="section"
    ...
    ><View class="hero-card"
      ...
      ><View class="hero-copy"
        ><Text class="hero-title">Hooks</Text><Text class="hero-body"
          >useFocusEffect, useIsFocused, and useNavigationState — introspecting the navigator's own live state from inside a screen.</Text
        ></View
      ></View
    ><Text testID="hooks-is-focused" class="info-text"
      >{`useIsFocused(): ${isFocused.current}`}</Text
    ><Text testID="hooks-focus-count" class="info-text"
      >{`useFocusEffect focus count: ${focusCount}`}</Text
    ><Text class="info-text"
      >{lastBlurAt === undefined
        ? 'not blurred yet'
        : `last blurred at ${lastBlurAt}`}</Text
    ><Text class="section-label"
      >useNavigationState() · current route stack</Text
    >{#each routeNames.current as name, index (`${name}-${index}`)}<Text
        class="list-row-text">{`${index}. ${name}`}</Text
      >{/each}</View
  ></SafeAreaView
>
