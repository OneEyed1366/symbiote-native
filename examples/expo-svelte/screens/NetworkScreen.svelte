<script lang="ts">
  ...
  const isConnectedText = $derived(yesNoLabel(networkState.current.isConnected));
  const isInternetReachableText = $derived(yesNoLabel(networkState.current.isInternetReachable));
  const ipAddressText = $derived(ipAddress === null ? PENDING_LABEL : ipAddress);
  ...
  ><ScrollView testID="network-scroll" class="screen" contentContainerStyle="scroll-content"
    ...
    ><View class="hero-card"
      ...
      ><View class="hero-copy"
        ><Text class="hero-title">Network</Text><Text class="hero-body">@symbiote-native/network — live network state via useNetworkState(), plus the device's IP address and airplane-mode check. Toggle Wi-Fi or airplane mode on the device to see the live card update on its own.</Text></View
      ></View
    ><View testID="network-live-card" class="network-card"
      ><Text class="network-card-title">Live network state</Text><View class="network-row"
        ><Text class="network-row-label">Type</Text><Text testID="network-type-value" class="network-value-text">{networkTypeText}</Text></View
      ><View class="network-row"
        ><Text class="network-row-label">Connected</Text><Text testID="network-connected-value" class="network-value-text">{isConnectedText}</Text></View
      ><View class="network-row"
        ><Text class="network-row-label">Internet reachable</Text><Text testID="network-reachable-value" class="network-value-text">{isInternetReachableText}</Text></View
      ></View
    ><View testID="network-info-card" class="network-card"
      ><Text class="network-card-title">Device info</Text><View class="network-row"
        ><Text class="network-row-label">IP address</Text><Text testID="network-ip-value" class="network-value-text">{ipAddressText}</Text></View
      ><View class="network-row"
        ><Text class="network-row-label">Airplane mode</Text><Text testID="network-airplane-value" class="network-value-text">{airplaneModeText}</Text></View
      ></View
    ></ScrollView
  ></SafeAreaView
>
