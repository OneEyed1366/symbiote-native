<script lang="ts">
  ...
  //
  // Markup whitespace is free in every screen of this app — format normally, do not pack tags
  ...
  const MENU_ITEMS: readonly IMenuItem[] = [
    ...
    },
    {
      ...
      hint: '@symbiote-native/file-system — legacy function API and the modern File/Directory/Paths class API',
    },
    {
      label: 'Audio',
      route: ROUTE_NAME.Audio,
      hint: '@symbiote-native/audio — playback, recording, and audio-session mode over a remote source',
    },
    {
      label: 'Notifications',
    ...
    },
    {
      ...
      hint: '@symbiote-native/task-manager + background-fetch + background-task — define and register a background task',
    },
    {
      label: 'SQLite',
      route: ROUTE_NAME.Sqlite,
      hint: '@symbiote-native/sqlite — SQLiteProvider/useSQLiteContext, CRUD, transactions, and a SQLite-backed key/value store',
    },
  ];
</script>
