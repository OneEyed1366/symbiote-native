  ...
  ><ScrollView testID="crypto-scroll" class="screen" contentContainerStyle="scroll-content"
    ...
    ><View class="hero-card"
      ...
      ><View class="hero-copy"
        ><Text class="hero-title">Crypto</Text><Text class="hero-body">@symbiote-native/crypto — cryptographically secure random bytes, randomUUID, and string digest hashing (SHA-1/256/384/512, MD2/4/5).</Text></View
      ></View
    ><View testID="crypto-uuid-card" class="crypto-card"
      ><Text class="crypto-card-title">Random UUID</Text><ActionButton testID="crypto-generate-uuid-button" title="Generate UUID" onPress={handleGenerateUuid} color={lineColor} />{#if uuidResult !== null}<View class="crypto-result-box"
        ><Text testID="crypto-uuid-result-value" class="crypto-result-text">{uuidResult}</Text></View
      >{/if}</View
    ><View testID="crypto-digest-card" class="crypto-card"
      ><Text class="crypto-card-title">Digest</Text><Text class="info-text">{`SHA-256 of "${DIGEST_SAMPLE_TEXT}"`}</Text><ActionButton testID="crypto-digest-sha256-button" title="Digest SHA-256" onPress={handleDigestSha256} color={lineColor} />{#if digestResult !== null}<View class="crypto-result-box"
        ><Text testID="crypto-digest-result-value" class="crypto-result-text">{digestResult}</Text></View
      >{/if}</View
    ><View testID="crypto-random-bytes-card" class="crypto-card"
      ><Text class="crypto-card-title">Random bytes</Text><ActionButton testID="crypto-get-random-bytes-button" title="Get 16 random bytes" onPress={handleGetRandomBytes} color={lineColor} />{#if randomBytesResult !== null}<View class="crypto-result-box"
        ><Text testID="crypto-random-bytes-result-value" class="crypto-result-text">{randomBytesResult}</Text></View
      >{/if}</View
    ></ScrollView
  ></SafeAreaView
>
