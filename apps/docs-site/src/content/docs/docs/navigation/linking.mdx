...
runs unless you wire it up yourself, and both work identically across all four adapters at the
...
All four adapters share the same semantics: on mount, the OS's initial URL is resolved once and
