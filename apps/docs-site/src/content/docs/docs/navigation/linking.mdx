...
runs unless you wire it up yourself, and both work identically across all five adapters at the
...
```ts
type IScreenLinkingConfig =
  string | { path?: string; screens?: Record<string, IScreenLinkingConfig> };
...
<Tabs syncKey="framework">
  ...
  <TabItem label="Svelte">
    ...
    ```svelte
    <script lang="ts">
      import { Stack, useLinkingIntegration } from '@symbiote-native/navigation/svelte';
      import type { INavigatorHandle } from '@symbiote-native/navigation/svelte';
      import { APP_LINKING_CONFIG } from './navigation-linking';

      // `Stack` ships as a raw `.svelte` file, so TypeScript resolves it through svelte's
      // ambient `declare module '*.svelte'` fallback — a bare component type with the
      // `export function` surface `bind:this` actually hands back erased. So the binding
      // target is typed `unknown` and narrowed on read instead of annotated directly.
    ...
    <Stack bind:this={stackInstance} initialRouteName="Home"><!-- screens --></Stack>
    ```

  </TabItem>
  <TabItem label="Solid">
    `createLinkingIntegration(config, navigatorHandle)` takes an **accessor**, not a resolved
    handle. `Stack`'s `ref` callback fires during the navigator's own creation, strictly before
    any effect runs, so by the time `createLinkingIntegration`'s internal `onMount` reads the
    accessor it has already resolved. No child-component workaround like React's, and unlike
    Svelte's `bind:this`, no runtime narrowing either: `ref` is already typed to hand back an
    `INavigatorHandle` directly:

    ```tsx
    import { createSignal } from 'solid-js';
    import { Stack, createLinkingIntegration } from '@symbiote-native/navigation/solid';
    import type { INavigatorHandle } from '@symbiote-native/navigation/solid';
    import { APP_LINKING_CONFIG } from './navigation-linking';

    function App() {
      const [stackHandle, setStackHandle] = createSignal<INavigatorHandle | null>(null);
...
All five adapters share the same semantics: on mount, the OS's initial URL is resolved once and
...
from the `@symbiote-native/navigation` package root — persistence _storage_ (e.g. AsyncStorage) is
...
async function persist(state: INavigatorState): Promise<void> {
  await AsyncStorage.setItem(
    'nav-state',
    JSON.stringify(serializeNavigatorState(state)),
  );
}
...
<Aside type="caution">
  Every route's `params` must be plain JSON-serializable data — no functions,
  class instances, or circular references. A route with a non-serializable param
  silently breaks the round trip: it either throws at `JSON.stringify` time, or
  worse, silently drops the offending field.
</Aside>
...
| `createLinkingIntegration(config: ILinkingConfig, navigatorHandle: Accessor<INavigatorHandle \| null>): void` (Solid) | Same behavior, but takes a Solid **accessor** over the Stack's `ref` target, read inside its own `onMount`, which runs after the navigator's `ref` callback has resolved           |
