...
<Aside type="note">
  ...
  **not** need to `pnpm add react-native-screens` yourself or import anything
  ...
  starts with `import '../register'`, a side-effect import that registers
  `RNSScreen`, `RNSScreenStack`, `RNSModalScreen`, `RNSScreenStackHeaderConfig`,
  `RNSScreenStackHeaderSubview`, `RNSSearchBar`, and `RNSScreenContentWrapper`
  in Fabric's view-config registry before any navigator is used. You only ever
  import from `@symbiote-native/navigation/react`, `/vue`, `/angular`, or
  `/svelte`.
</Aside>
