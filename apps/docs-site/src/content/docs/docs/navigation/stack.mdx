...
<Aside type="caution">
  Unlike react-navigation, `INavigatorHandle` has **no `.navigate()` method**. `push` always stacks
  a new route instance; `replace` swaps the current top route for a new one. Pick whichever of the
  two matches the transition you want.
</Aside>
