...
<Tabs syncKey="framework">
  ...
  <TabItem label="Svelte">
    ...
    <ActionButton title="Go to Details" onPress={() => navigation.current.push('Details', { id: 42 })} />
    ```
    ...
    <ActionButton title="Back" onPress={() => navigation.current.pop()} />
    ```
    ...
    `<Screen>`'s `component` prop takes a compiled Svelte `Component` (an ordinary `.svelte`
  ...
  <TabItem label="Solid">
    ...
    export function HomeScreen() {
      ...
      return (
        <ActionButton
          title="Go to Details"
          onPress={() => navigation().push('Details', { id: 42 })}
        />
      );
    }
    ```
    ...
    `<Stack.Screen>`'s `component` prop takes a Solid `Component` reference, not a render prop:
    the navigator mounts it itself. Solid's JSX takes the dotted `Stack.Screen` form directly (no
    Vue/Svelte-style standalone `Screen` needed); markers register through a context-based
    collector rather than a children scan, since Solid has no way to inspect `children` either.
    `useRoute()`/`useStackNavigation()` hand back an **accessor**, called at the read site
    (`navigation().push(...)`, never destructured): a Solid component body runs once, so a
    snapshot taken at the top would freeze and never see a later `setParams`. Any component under
    the stack reads them the same way, at any nesting depth. See
    [Hooks & focus](/docs/navigation/hooks/).
...
<Tabs syncKey="framework">
  ...
  <TabItem label="Svelte">
    ...
    <!-- <Stack bind:this={stackInstance}>...</Stack> -->
    {#if isNavigatorHandle(stackInstance)}
      <!-- stackInstance.push('Details') -->
    {/if}
    ```

    Unlike Vue's `ref<INavigatorHandle | null>`, a Svelte `bind:this` target for `Stack` types as
  ...
  <TabItem label="Solid">
    ```tsx
    let stackHandle: INavigatorHandle | undefined;
    // <Stack ref={(handle) => (stackHandle = handle)}>...</Stack>
    stackHandle?.push('Details');
    ```

    `Stack`'s `ref` is a plain callback prop (`(handle: INavigatorHandle) => void`), Solid's own
    spelling of "give me the thing when it exists", not a signal or a `bind:this` target like
    Vue's/Svelte's. The callback fires once, synchronously, from the navigator's body.
...
<Aside type="caution">
  Unlike react-navigation, `INavigatorHandle` has **no `.navigate()` method**.
  `push` always stacks a new route instance; `replace` swaps the current top
  route for a new one. Pick whichever of the two matches the transition you
  want.
</Aside>
