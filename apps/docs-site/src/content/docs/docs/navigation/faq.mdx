...
reads them with `useNavigation()`/`useRoute()` in React, Vue, Svelte, and Solid, or
`injectNavigation()`/`injectRoute()` in Angular. Better yet, reach for the navigator-specific
narrowed variant (`useStackNavigation()`/`useTabNavigation()`/`useDrawerNavigation()`, or
Angular's `injectStackNavigation()`/etc.) so the handle comes back concretely typed with nothing
to narrow. The navigator does not pass `route`/`navigation` in as a prop or `@Input()`; the hook
is the only path, and it reads the same at any nesting depth. `useIsFocused()`/`useFocusEffect()`/
`useNavigationState()` work the same way in React, Vue, and Svelte; Solid names the same three
`createIsFocused()`/`createFocusEffect()`/`createNavigationState()`, matching `createSignal`'s
naming since each owns a signal or subscription, and Angular names them
`injectIsFocused()`/`injectFocusEffect()`/`injectNavigationState()`. See
...
Vue, Svelte), `createFocusEffect` (Solid), or `injectFocusEffect` (Angular),
or the boolean-returning `useIsFocused`/`createIsFocused`/`injectIsFocused`,
to run code on every focus instead of once. In React, wrap the callback you
pass to `useFocusEffect` in `useCallback` — a new callback identity on every
render re-subscribes the effect — every other adapter's equivalent doesn't
need this. See [Hooks & focus](/docs/navigation/hooks/).
...
free. Supply your own drawer content: `renderDrawerContent` prop on React and
Solid, Vue's `drawerContent` scoped slot, Svelte's `drawerContent` snippet, or
Angular's `#drawerContent` template. See [Drawer navigator](/docs/navigation/drawer/).
