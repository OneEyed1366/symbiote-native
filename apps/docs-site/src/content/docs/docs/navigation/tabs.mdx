...
<Aside type="caution">
  Only the **focused** route's screen is ever mounted. Switching tabs unmounts the previous screen's
  component and mounts the new one from scratch — its state does not survive a switch away and back,
  and its constructor/setup re-runs every time it regains focus. If you need to react to that, see
