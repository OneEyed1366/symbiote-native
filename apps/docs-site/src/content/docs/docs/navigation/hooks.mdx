---
title: Hooks & focus
description: Reading the current route/navigator and reacting to screen focus from React, Vue, Angular, and Svelte.
---
...
function names are identical to React's/Vue's. All four read from the nearest enclosing
...
<Aside type="tip">
  ...
  a `$derived`/template/`$effect` — the same unwrap pattern as Vue's `ComputedRef.value`.
</Aside>
...
<Aside type="caution">
  React's `useFocusEffect` must be given a **memoized** callback (wrap it in `useCallback`) — a new
  function identity on every render re-subscribes the effect, same as any other `useEffect`
  dependency. Vue's `useFocusEffect`, Angular's `injectFocusEffect`, and Svelte's `useFocusEffect`
  need no memoization: `setup()`, a component constructor, and a Svelte component's script each run
  exactly once, so the callback is captured a single time by value.
</Aside>
...
`navigation.current.getParent()`; Angular's reads `injectTabNavigation().getParent()` — same
one-hop semantics in all three.
