...
<Aside type="caution">
  ...
  Angular's `injectFocusEffect`, and Svelte's `useFocusEffect` need no
  memoization: `setup()`, a component constructor, and a Svelte component's
  script each run exactly once, so the callback is captured a single time by
  value.
</Aside>
