...
<Aside type="caution">
  Drawer reaches the same swipe-to-open/close and `front`/`back`/`slide`/`permanent` behavior as
  `@react-navigation/drawer`, but not byte-for-byte parity with it. `PanResponder` + `Animated`
  cover the gesture and the transform math; a few pieces of the real drawer's surface are not
  ported: no `configureGestureHandler` escape hatch, no declarative gesture-relationship composition
  against a nested `ScrollView` or another `PanResponder`, no UI-thread `useDrawerProgress`
  `SharedValue` (`progress` here is a plain JS-thread value), and
  `hideStatusBarOnOpen`/`keyboardDismissMode`/`statusBarAnimation`/`overlayStyle` are not wired up.
  This is a deliberate scope boundary, not a bug.
</Aside>
