...
<Aside type="caution">
  Drawer reaches the same swipe-to-open/close and
  `front`/`back`/`slide`/`permanent` behavior as `@react-navigation/drawer`, but
  not byte-for-byte parity with it. `PanResponder` + `Animated` cover the
  gesture and the transform math; a few pieces of the real drawer's surface are
  not ported: no `configureGestureHandler` escape hatch, no declarative
  gesture-relationship composition against a nested `ScrollView` or another
  `PanResponder`, no UI-thread `useDrawerProgress` `SharedValue` (`progress`
  here is a plain JS-thread value), and
  `hideStatusBarOnOpen`/`keyboardDismissMode`/`statusBarAnimation`/`overlayStyle`
  are not wired up. This is a deliberate scope boundary, not a bug.
</Aside>
...
Angular, a snippet in Svelte, a render prop over an accessor in Solid.
...
<Tabs syncKey="framework">
  <TabItem label="React">
    ...
    function renderDrawerContent({ state, descriptors, navigation }: {
      ...
      return (
        <safe-area-view className="drawer-panel">
    ...
    <template>
      <Drawer initial-route-name="Home" drawer-position="right" drawer-type="slide">
        ...
        <template #drawerContent="{ state, descriptors, navigation }">
          <safe-area-view class="drawer-panel">
  ...
  <TabItem label="Svelte">
    ...
    <Drawer initialRouteName="Home" drawerPosition="right" drawerType="slide"
      ...
      />{#snippet drawerContent(slot: IDrawerContentSlotProps)}<safe-area-view class="drawer-panel"
        >{#each slot.state.routes as route (route.key)}<pressable
          onPress={() => slot.navigation.jumpTo(route.name)}
          ><text>{slot.descriptors[route.key]?.options.drawerLabel ?? route.name}</text></Pressable
        >{/each}</SafeAreaView
      >{/snippet}</Drawer
    >
    ```

    `Drawer`'s content markup is a `Snippet` **with a parameter** (`drawerContent`), read from
    ...
    mechanism [Stack's `<Screen>`](/docs/navigation/stack/) uses.
  ...
  <TabItem label="Solid">
    ...
    import type { IDrawerContentSlotProps } from '@symbiote-native/navigation/solid';
    import HomeScreen from './HomeScreen';
    import SettingsScreen from './SettingsScreen';

    function renderDrawerContent(slot: Accessor<IDrawerContentSlotProps>) {
      return (
        <safe-area-view class="drawer-panel">
          <For each={slot().state.routes}>
            {route => (
              <pressable onPress={() => slot().navigation.jumpTo(route.name)}>
                <text>{slot().descriptors[route.key]?.options.drawerLabel ?? route.name}</text>
              </pressable>
            )}
          </For>
        </safe-area-view>
    ...
    export function DrawerDemoScreen() {
      return (
        <Drawer initialRouteName="Home" drawerPosition="right" drawerType="slide" drawerContent={renderDrawerContent}>
    ...
    `drawerContent` is a render prop over an **accessor** (`(slot: Accessor<IDrawerContentSlotProps>)
    => JSX.Element`), Solid's twin of React's render-prop and Vue's scoped slot. It's called once and
    untracked internally, so read `slot().state`/`slot().descriptors`/`slot().navigation`, never
    destructure `state`/`descriptors` at the callback's own top level - that freezes on an empty
    navigator (the `<Drawer.Screen>` markers below haven't registered by the time the callback runs).
    `Drawer` reads `<Drawer.Screen>` markers off a context-based collector, the same registration
    mechanism [Stack's `<Screen>`](/docs/navigation/stack/) uses.
...
React/Vue/Svelte/Solid or `injectDrawerNavigation()` in Angular: the narrowed twin of
...
`bind:this` in Svelte, a callback `ref` prop in Solid), identical across all five:
...
| Solid     | `drawerContent` render prop       | `(slot: Accessor<IDrawerContentSlotProps>) => JSX.Element` passed as the `drawerContent` prop on `<Drawer>`; called once, untracked, read `slot()` inside, not at the callback's own top level |
...
`injectNavigation()`/`injectRoute()`/`injectIsFocused()`/`injectFocusEffect()`; Solid:
`useNavigation()`/`useRoute()`/`createIsFocused()`/`createFocusEffect()`)](/docs/navigation/hooks/)
across all five adapters, including how to narrow a navigation handle down
