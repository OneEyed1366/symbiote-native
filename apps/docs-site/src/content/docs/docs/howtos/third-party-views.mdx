...
<Aside type="danger" title="The library's component is React-only">
  ...
  library's underlying **native view** can be made framework-agnostic — never
  its React component wrapper.
</Aside>
...
const nativeLibRoot = path.dirname(
  require.resolve('@react-native-community/slider/package.json'),
);
    ...
    platforms: {
      android: {
        sourceDir: path.relative(
          cliDependencyRoot,
          path.join(nativeLibRoot, 'android'),
        ),
