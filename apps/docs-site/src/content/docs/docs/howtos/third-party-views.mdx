...
<Aside type="danger" title="The library's component is React-only">
  A third-party RN package ships a React **component** whose body calls hooks off the React
  dispatcher. Rendering it from Vue or Angular hits a null dispatcher and throws `Cannot read
  property 'useState' of null`. Only the library's underlying **native view** can be made
  framework-agnostic — never its React component wrapper.
</Aside>
...
<Tabs syncKey="framework">
  ...
  <TabItem label="Svelte">
    ...
    <Slider value={0.5} onValueChange={next => (value = next)} />
    ```
...
directly. (That's the rule for native _view_ wrappers, which go through React
Native's own CLI autolinking. A wrapper over an `expo-modules-core` module
registers through a different path — see [Wire up an Expo native
module](/docs/howtos/expo-native-module-setup/).) That means the wrapper itself has to look autolinkable to React
...
`jsSrcsDir` at the _nested_ copy (`node_modules/<native-lib>/src`) so RN's own
