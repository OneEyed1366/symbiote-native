...
<Aside type="tip" title="Automatic, as of @symbiote-native/expo-modules-link">
  This line used to be a hand-edit. If the package you're consuming (or authoring) depends on
  `@symbiote-native/expo-modules-link` and ships a `native-link.json` manifest plus a
  `postinstall` script calling `symbiote-expo-link`, it's patched in for you the first time you
  `npm install` — see step 7 for the manifest shape and what actually gets generated.
</Aside>
...
<Aside type="tip" title="Automatic, as of @symbiote-native/expo-modules-link">
  There's still no `expo` aggregator project to auto-generate this module list, and
  `ModuleRegistryAdapter` still only accepts a single `ModulesProvider` — so every
  expo-modules-core package wired into the app lands its own entry in the same shared map. What
  changed is who writes that entry: a wrapper package that depends on
  `@symbiote-native/expo-modules-link` and ships a `native-link.json` manifest gets it (and the
  `build.gradle` line from the previous step) patched in automatically, the first time you
  `npm install` it.
</Aside>

The manifest lives next to the wrapper package's own `package.json`. Here's
...
Wiring a *new* wrapper package into this yourself needs two `package.json` edits — a dependency
on `@symbiote-native/expo-modules-link` (`npm install @symbiote-native/expo-modules-link` adds
it) and one script:
...
If you're only *consuming* an already-published package like `@symbiote-native/sensors` in your
own app, there's nothing to do here at all — its own `postinstall` runs this for you.

What actually lands in your app's `MainApplication.kt` looks like this (a real excerpt — the
patcher is additive-only, so every package's postinstall only ever appends a line under the
marker, never rewrites one):
...
// ...one import per module, appended as each package's postinstall runs
...
The platform permission string used to be yours to declare by hand; it's now registered
automatically the same way as the Android module itself (see step 7) — a package that needs one
declares it in its own `native-link.json` under `ios.infoPlistKeys`, and `symbiote-expo-link`
inserts a generic default into `Info.plist` on `npm install` if that key isn't already there.
Add your own wording to `Info.plist` yourself, before or after install, to override the default
permanently — the patcher never touches a key that already exists.
