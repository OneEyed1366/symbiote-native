...
<Aside type="tip" title="Generated by @symbiote-native/expo-modules-link">
  ...
  changed is who writes that entry: the app-level `symbiote-expo-link` run from step 6 owns two
  regions in this file (the imports and the map) and rewrites both on every install, alongside the
...
Authoring a *new* wrapper package needs nothing beyond that manifest — no `postinstall`, no
