...
description: Wiring the same native event across React, Vue, Angular, and Svelte.
---
...
<Aside type="caution" title="Scroll events are the one exception">
  ...
  stay plain callback `@Input()`s on Angular, even on `ScrollView` and the lists — RN's scroll
  callbacks can carry an `Animated.event(...)` marker, and `@Output()` can only bind a template
