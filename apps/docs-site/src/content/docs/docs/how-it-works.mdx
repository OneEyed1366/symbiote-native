---
title: How it works
description: The SymbioteJS render pipeline from framework adapters to the stock React Native Fabric host.
---

SymbioteJS is built around one architectural fact: Fabric already exposes a
framework-agnostic JavaScript seam.

React Native's React renderer talks to this seam from its Fabric host config.
SymbioteJS talks to the same seam from its own engine.
...
SymbioteJS does not fork React Native native code. These stay ordinary React Native
runtime pieces:
...
that handle is a Fiber. In SymbioteJS, it is a retained-tree node owned by the
