...
clip, and the canary has been benchmarked on a real iOS simulator repeatedly
(see the root `CLAUDE.md`'s device numbers); what's still missing is the
shared Detox `canary-journeys` e2e spec React, Vue, and Svelte already carry
— `examples/solid` has no `e2e/` harness yet.
