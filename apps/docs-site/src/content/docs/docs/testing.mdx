...
<Aside type="tip" title="One spec file, shared across React and Vue">
  Detox attaches to the stock React Native host **below** the layer SymbioteNative replaces — it
  drives the real native views through Fabric's own accessibility tree, the same way it would for a
  plain React Native app. The same `canary-journeys` e2e spec runs unmodified across
  `examples/react`, `examples/vue-sfc`, and `examples/vue-tsx`, as long as each exposes the same
  `testID`s. A component that mounts but behaves wrong under one adapter — say, a native-driven
  `Animated` view that renders but never actually animates — fails the same assertion the same way
  regardless of which framework rendered it. `examples/angular` has its own Detox e2e harness today
  but doesn't yet share that exact spec file.
</Aside>
