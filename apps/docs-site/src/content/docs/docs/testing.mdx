...
<Aside type="tip" title="One spec file, shared across React and Vue">
  ...
  `examples/vue-sfc`, and `examples/vue-tsx`, as long as each exposes the same
  `testID`s. A component that mounts but behaves wrong under one adapter — say,
  a native-driven `Animated` view that renders but never actually animates —
  fails the same assertion the same way regardless of which framework rendered
  it. `examples/angular` has its own Detox e2e harness today but doesn't yet
  share that exact spec file.
</Aside>
