...
<Aside type="danger" title="Native setup is required before first use">
  `expo-sms`'s native code is discovered by `expo-modules-autolinking`, a different mechanism from
  the `react-native.config.cjs`/podspec autolinking every other SymbioteNative wrapper uses — and
  it isn't the standard Expo setup flow either, since this project never installs the `expo`
...
`@symbiote-native/sms` ships zero React/Vue/Angular logic — `expo-sms`'s own JS is hand-ported
into this package's `core/`, resolving the native module through `expo-modules-core`'s
