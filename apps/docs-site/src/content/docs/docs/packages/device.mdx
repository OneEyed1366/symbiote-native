...
<Tabs syncKey="framework">
  ...
  <TabItem label="Solid">
    ...
    The constants (`isDevice`, `brand`, `modelName`, `osName`, `osVersion`, `deviceName`) are read
    once at import time and need no signal - only the two async calls do. There's no per-instance
    primitive to reach for here either - every constant/function is a plain export off the core
    package.
...
`@symbiote-native/device` ships zero React/Vue/Angular/Svelte/Solid logic in `expo-device` itself — its
...
├── solid/    # @symbiote-native/device/solid   — export * from '../core'
