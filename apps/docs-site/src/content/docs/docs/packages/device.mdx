...
<Aside type="danger" title="Native setup is required before first use">
  `expo-device`'s native code is discovered by `expo-modules-autolinking`, a different mechanism
  from the `react-native.config.cjs`/podspec autolinking every other SymbioteNative wrapper
  uses — and it isn't the standard Expo setup flow either, since this project never installs the
...
<Aside type="note" title="Every function except the platform-feature pair throws when unavailable">
  Every function above throws `UnavailabilityError` when its native method is missing, **except**
  `getPlatformFeaturesAsync`/`hasPlatformFeatureAsync`, which resolve to `[]`/`false` instead — a
  platform-feature query on a platform with no such concept (iOS) is a normal "no" answer, not an
  error, matching upstream's own behavior.
</Aside>
