...
per-instance state, so there is no hook/composable/service to wrap — the React, Vue, Angular, and
Svelte entry points are plain re-exports of the same `core`.
...
All four adapters — React, Vue, Angular, Svelte — re-export the exact same functions; there is no
