...
<Aside type="note" title="React/Vue/Angular/Svelte have no shared default-tag mechanism">
  ...
  other's activation/deactivation. Vue, Angular, and Svelte have no `useId` equivalent, so all
  three fall back to a small monotonically-incrementing module-local counter instead —
  functionally equivalent, just not identical string values across adapters.
</Aside>
