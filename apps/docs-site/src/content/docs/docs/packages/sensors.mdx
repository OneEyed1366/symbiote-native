...
<Aside type="danger" title="Native setup is required before first use">
  ...
  Pedometer need an `NSMotionUsageDescription` string on iOS; `symbiote-expo-link` inserts a
  generic default when that key is absent, and leaves your own wording alone if you write one.
</Aside>
