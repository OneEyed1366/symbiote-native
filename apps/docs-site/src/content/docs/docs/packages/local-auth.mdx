...
<Aside type="danger" title="Native setup is required before first use">
  `expo-local-authentication`'s native code is discovered by `expo-modules-autolinking`, a
...
All three adapters — React, Vue, Angular — re-export the exact same free functions; there is no
...
| `authenticateAsync(options?: ILocalAuthenticationOptions): Promise<ILocalAuthenticationResult>` | Attempts to authenticate via Fingerprint/TouchID, or FaceID where available; without `NSFaceIDUsageDescription` set in `Info.plist`, iOS falls back to the device passcode instead of throwing |
...
Unlike sensors' `react/hooks`, `vue/composables`, and `angular/services` folders — each full of
per-sensor lifecycle wrappers — local-auth's three adapter entries are single-file re-exports
with no lifecycle code at all: every function here is stateless and one-shot, so there is nothing
for a hook, composable, or service to subscribe to or clean up. The native code itself is never
