...
<Aside type="danger" title="Two setup steps, not one">
  ...
  *before* JS runs) has to be generated and wired into your native project files first. Follow [How
  to: add a native splash screen](/docs/howtos/splash-screen/) for the full asset-generation +
...
<Aside type="note">
  ...
  images (if you asked for them) have loaded, and your own `ready` flag is true — a faithful port of
...
<Tabs syncKey="framework">
  ...
  <TabItem label="Svelte">
    ...
    <script lang="ts">
      ...
      // Animated.View is dotted, so it can't be a template tag — aliased first, matching
      // examples/svelte's own AnimatedDemo.svelte convention.
    ...
    ><Image {...hideAnimation.current.logo} /></AnimatedView>
    ```
