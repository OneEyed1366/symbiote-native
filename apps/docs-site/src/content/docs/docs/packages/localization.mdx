...
<Aside type="danger" title="Native setup is required before first use">
  `expo-localization`'s native code is discovered by `expo-modules-autolinking`, a different
...
| --- | --- | --- |
| (return value) | `Locale[]` | React returns this as a plain array, recomputed via `useMemo` whenever the locale-change listener fires. Vue returns `Ref<Locale[]>`. Angular's `LocalesService.connect()` returns `Signal<Locale[]>` |

### `useCalendars()` return value

| Field | Type | Description |
| --- | --- | --- |
| (return value) | `Calendar[]` | React returns this as a plain array, recomputed via `useMemo` whenever the calendar-change listener fires. Vue returns `Ref<Calendar[]>`. Angular's `CalendarsService.connect()` returns `Signal<Calendar[]>` |
...
<Aside type="note" title="getLocales/getCalendars are synchronous">
  Unlike most native-module calls in this repo, both getters are direct, synchronous, JSI-bridged
  native calls — no `await`, no `Promise`. That's why Vue's `useLocales`/`useCalendars` seed their
  `ref` directly at setup time (`ref(getLocales())`) instead of an async placeholder-then-fetch —
  there is no "loading" state to represent.
</Aside>
