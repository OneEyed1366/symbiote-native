...
<Aside type="danger" title="Native setup is required before first use">
  ...
  wrapper uses — and it isn't the standard Expo setup flow either, since this project never installs
  the `expo` meta-package. Follow [How to: wire up an Expo native
