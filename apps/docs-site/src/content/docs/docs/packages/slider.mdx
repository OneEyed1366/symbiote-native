...
SymbioteJS adapter can render it — without importing that library's React component body. It is
...
<Aside type="caution">
  ...
  `@symbiote-native/slider/react` or `@symbiote-native/slider/vue`, never from the community package itself.
</Aside>
...
explicit form. See the [Vue API reference](/symbiote-native/docs/api/vue/#model-bindings-v-model).
...
a Vue scoped slot) — see [shared vs framework-specific](/symbiote-native/docs/api/components/#shared-vs-framework-specific).
...
same `core` render — the same logic/view/lifecycle split as every other SymbioteJS component (see
[how it works](/symbiote-native/docs/how-it-works/)). Wrapping a different third-party native view follows this
exact recipe; see
[decision 0027](https://github.com/OneEyed1366/symbiote-native/blob/main/.docs/decisions/0027-third-party-native-view-wrapper-package.md)
