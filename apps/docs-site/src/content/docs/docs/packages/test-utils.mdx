...
<Aside type="caution" title="Install once per file, before the first commit — not once per test">
  ...
  destructures the slot's methods on its first commit and caches them for the rest of the process.
  So every suite in this repo calls `installFabric()` a single time at module scope and `reset()`s
  between tests — a second `installFabric()` after anything has committed replaces the global, but
  the engine keeps writing to the handle it already bound, and the new recorder stays silently
  empty. Vitest isolates test files from one another, so per-file is per-clean-slate; within a file,
  `reset()` is what gives each test a clean recorder.
</Aside>
