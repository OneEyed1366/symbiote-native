...
`@symbiote-native/css-parser` compiles a stylesheet — a `.css`/`.module.css` file, an inline
`<style>` block in a Vue SFC or a Svelte component, or an SCSS/Sass/Less/Stylus source — into
React Native style objects at build
