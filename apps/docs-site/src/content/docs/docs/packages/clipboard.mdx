...
<Aside type="caution" title="The URL functions are iOS-only">
  ...
  Android implementation for URL-typed clipboard content. Text, image, and the change listener work
...
<Aside type="danger" title="Native setup is required before first use">
  `expo-clipboard`'s native code is discovered by `expo-modules-autolinking`, a different mechanism
