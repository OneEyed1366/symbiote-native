...
<Aside type="caution" title="Two documented APIs are dead on Android without this package">
  [`Keyboard`](/docs/api/react/) and [`Settings`](/docs/api/react/) are listed as available on
  every adapter's API page, and they are — on iOS unconditionally, on Android only once this
  package is installed. Missing it produces no crash and no error: `Keyboard.addListener` returns
  a real subscription that never fires, and `Settings.get` reads from an empty snapshot. If either
  API looks inert on an Android build, check this dependency first.
</Aside>
...
- **The IME inset is read directly, not via `getRootWindowInsets()`.** Under `adjustResize` the
  latter's *consumed* insets read `0` while the keyboard is up. The module observes the raw
