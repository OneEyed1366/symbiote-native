...
All three adapters are listed as live because none of them import this package. They reach it
...
Both are re-exported by [React](/docs/api/react/), [Vue](/docs/api/vue/), and
[Angular](/docs/api/angular/) from `@symbiote-native/engine`, so the JS is identical across all
three and across both platforms.
