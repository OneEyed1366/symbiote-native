...
hook/composable/service to wrap — the React, Vue, and Angular entry points are plain re-exports of
the same `core`.
...
<Aside type="danger" title="Native setup is required before first use">
  `expo-system-ui`'s native code is discovered by `expo-modules-autolinking`, a different
...
Both adapters — React, Vue, Angular — re-export the exact same two functions; there is no
