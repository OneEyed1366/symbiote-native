...
All four adapters — React, Vue, Angular, Svelte — re-export the exact same default export and
named function; there is no per-adapter hook/composable/service to reach for, since nothing here
