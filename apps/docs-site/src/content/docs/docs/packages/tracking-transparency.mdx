...
returns both as getters, read as `permissions.status` / `permissions.error`; Solid returns both as
accessors, `status()` / `error()` - the auto-fetch runs once, synchronously, inside the
primitive's own body rather than a mount effect, since a Solid component body only runs once.
...
└── solid/primitives/      @symbiote-native/tracking-transparency/solid  — createPermissions
```
