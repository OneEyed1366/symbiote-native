...
<Aside type="note" title="Android has no native expoDidUpdateDimensions event">
  ...
  re-fetching the lock and orientation on every dimensions change. iOS and web both subscribe to the
  native `expoDidUpdateDimensions` event directly.
</Aside>
...
<Tabs syncKey="framework">
  ...
  <TabItem label="Svelte">
    ```svelte
    <script lang="ts">
      import { Button, Text } from '@symbiote-native/svelte';
      import { lockAsync, OrientationLock } from '@symbiote-native/screen-orientation';
      import { useScreenOrientation } from '@symbiote-native/screen-orientation/svelte';

      const screenOrientation = useScreenOrientation(); // { readonly current: ScreenOrientationState }

      function onLockLandscape(): void {
        void lockAsync(OrientationLock.LANDSCAPE);
      }
    </script>
    ...
    <Button title="Lock landscape" onPress={onLockLandscape} />
    ```
...
`ScreenOrientationService.connect()` returns `Signal<ScreenOrientationState>`; Svelte returns
`{ readonly current: ScreenOrientationState }`, a boxed getter read as `.current`.
