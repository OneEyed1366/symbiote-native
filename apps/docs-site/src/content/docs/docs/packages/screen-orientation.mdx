...
<Aside type="note" title="Android has no native expoDidUpdateDimensions event">
  `expo-screen-orientation` doesn't emit its own `expoDidUpdateDimensions` event
  on Android — the module piggybacks on RN's own
  `Dimensions.addEventListener('change', ...)` there instead, re-fetching the
  lock and orientation on every dimensions change. iOS and web both subscribe to
...
<Aside type="danger" title="Native setup is required before first use">
  `expo-screen-orientation`'s native code is discovered by
...
<Tabs syncKey="framework">
  ...
  <TabItem label="Solid">
    ```tsx
    import { Button, Text } from '@symbiote-native/solid';
    import { lockAsync, OrientationLock } from '@symbiote-native/screen-orientation';
...
`@symbiote-native/screen-orientation` ships zero React/Vue/Angular/Svelte/Solid logic in
