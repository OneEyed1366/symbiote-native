---
title: Vue guide
description: How to write SymbioteJS apps with the Vue adapter.
---
...
<img src={`${import.meta.env.BASE_URL}/vue-demo.gif`} alt="Vue SFC driving a native SymbioteJS demo" width="260" />

*Vue SFC (`.vue`) — the reference canary at `examples/vue-sfc`.*

<img src={`${import.meta.env.BASE_URL}/vue-tsx-demo.gif`} alt="Vue TSX driving a native SymbioteJS demo" width="260" />
...
[Styling guide](/symbiote-native/docs/learn/styling/) for both paths.
...
- controlled-value components (`Switch`, `TextInput`, the [Slider](/symbiote-native/docs/packages/slider/)
  wrapper) support real Vue `v-model`, alongside the explicit `value`/`@change-*` pair;
- `v-show` works on any SymbioteJS host node — it toggles native `display: none`
...
Read the exact API surface in the [Vue API reference](/symbiote-native/docs/api/vue/).
