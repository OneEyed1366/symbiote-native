...
SymbioteJS ships a from-scratch, framework-agnostic port of React Native's
...
SymbioteJS's `Animated` is the RN `Animated` API, not Reanimated — there is no
...
itself is not tested against SymbioteJS's engine; it hooks much deeper into RN's
runtime than a plain native-view library does, so treat it as unverified
rather than assuming it works the same way a
[wrapped third-party native view](/symbiote-native/docs/howtos/third-party-views/) would.
