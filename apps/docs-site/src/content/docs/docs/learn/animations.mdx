...
<Aside type="caution" title="Two different reasons for one workaround">
  Vue, Svelte, and Angular all need a plain identifier instead of `Animated.View`, but not for the
  same reason. Vue and Svelte templates simply can't parse a dotted tag name, so both alias it to a
  local const before use. Angular's reason is deeper: `ngc`'s AOT partial-mode compiler statically
  evaluates template type-checking and can't trace a component class through property access on an
  external, pre-compiled namespace object — only through a direct named import binding.
  `@symbiote-native/angular` exports `AnimatedView`, `AnimatedText`, `AnimatedImage`,
  `AnimatedScrollView`, `AnimatedFlatList`, and `AnimatedSectionList` as top-level named symbols for
  exactly this reason; plain `tsc`/Vitest won't catch a dotted alias slipping through, only a real
  `ngc` build does.
</Aside>
...
is the one directive in that family that _is_ legal on a component, and it's your
