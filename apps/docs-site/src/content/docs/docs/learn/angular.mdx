---
title: Angular guide
description: How to write SymbioteJS apps with the Angular adapter.
---
...
<Aside type="caution" title="One deliberate exception">
  ...
  packages have followed: [Slider](/symbiote-native/docs/packages/slider/) ships an Angular
  build too — see the [status page](/symbiote-native/docs/project/status/) for the exact
  line.
</Aside>
...
<img src={`${import.meta.env.BASE_URL}/angular-demo.gif`} alt="Angular driving a native SymbioteJS demo" width="260" />
...
inline instead of a CSS file — see the [Styling guide](/symbiote-native/docs/learn/styling/)
for both paths.
...
- every SymbioteJS component is `standalone: true` — import it directly in the
...
- imperative handles use `@ViewChild(..., { read: ElementRef })` — the
  `nativeElement` is the SymbioteJS host node, not a DOM element;
...
Read the exact API surface in the [Angular API reference](/symbiote-native/docs/api/angular/).
