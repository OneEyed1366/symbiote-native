...
Angular — real `@Output()` `EventEmitter`s everywhere, including `Switch` and
`TextInput`:
...
<Aside type="caution" title="Scroll events stay callback inputs">
  ...
  value. See the [Angular API reference](/symbiote-native/docs/api/angular/) for the exact
  split.
</Aside>
...
SymbioteJS commands native back to the JavaScript value.
