...
Every component below ships on React, Vue, Angular, and Svelte. What differs is
the framework-shaped surface:
...
- `View` — React uses `children`; Vue uses the default slot; Svelte takes a
  ...
  `TextInput`/`ScrollView`'s `focus`/`scrollTo`).
- `Text` — Text children differ by framework; press events follow callback vs
  ...
  (no `IResponderProps`, matching every other adapter's `ITextProps`).
...
- `ScrollView` — Imperative handle, refresh, and sticky behavior need
  ...
  scroll offset via Svelte context).
- `Pressable` — React uses a render child; Vue uses a scoped slot; Svelte uses
  ...
  React — Svelte never translates to an emit/Output shape.
...
- `Switch` — `onValueChange` maps to `@value-change` on Vue and `(valueChange)`
  ...
  parent rejects) is shared logic.
- `ActivityIndicator` — Mostly shared render-only props. Svelte calls
  ...
  hand-authoring the spinner tag.
...
- `Animated` — The value graph is shared; component wrapping differs by
  ...
  native-thread `useNativeDriver` animation.
...
| `Switch`    | `onValueChange` | `@value-change`<br />or `v-model` | `(valueChange)` | `onValueChange` | `boolean`        |
| `TextInput` | `onValueChange` | `@value-change`<br />or `v-model` | `(valueChange)` | `onValueChange` | `string`         |
...
begin with: the callback prop name is RN's own name, unchanged.
...
value](/docs/howtos/two-way-binding/) for the full picture.
...
runes) and executes the effects.
...
redeclaring the whole shape — React adds `className?: string`, Vue and
Svelte both add `class?:` (Svelte's is `ISvelteClassValue`, a wider type that
additionally accepts a clsx-style object/array, resolved by the adapter
itself before it reaches the engine). `IImageProps`, `ITextInputProps`,
