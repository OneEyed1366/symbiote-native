...
- `TextInput` — `onValueChange` maps to `@value-change` on Vue and `(valueChange)`
  ...
  translation layer. The controlled write handshake (event-count-gated
  `setTextAndSelection` command) is shared logic across all four.
- `Switch` — `onValueChange` maps to `@value-change` on Vue and `(valueChange)`
  ...
  React-shaped. Controlled snap-back (`dispatchViewCommand` when native
  reports a value the parent rejects) is shared logic.
...
with `bind:value={x}` at the call site) — a supported authoring pattern for
your own components. The shipped `TextInput`/`Switch` do not use it: they
keep the same controlled `value` + `onValueChange` callback handshake React
uses, so a Svelte app writes `<TextInput value={x} onValueChange={setX} />`,
not `bind:value`.
