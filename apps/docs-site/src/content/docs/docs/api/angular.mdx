...
Every SymbioteJS component is a `standalone: true` Angular component — add it to
...
| Refs            | `@ViewChild('host', { read: ElementRef })`, `.nativeElement` is the SymbioteJS host node |
...
[Core API](/symbiote-native/docs/api/core/#app-entry-point-appregistry). One Angular-specific
...
[Animations guide](/symbiote-native/docs/learn/animations/) for the full surface and why the
named exports exist.
...
the whole tree isn't a SymbioteJS quirk, it's why demo screens are split into
...
instead — see the [Slider package](/symbiote-native/docs/packages/slider/) for the reference
