...
description: Controlled text input in React, Vue, Angular, and Svelte.
---
...
| ----------------- | ---------------------------- | --------------------------------- | ----------------------------------------------------- | -------------------------------------------------- |
| Text prop         | `value`                      | `:value`<br />or `v-model`        | `[value]`<br />or `[(value)]`                         | `value`                                            |
| Text change       | `onValueChange(text, event)` | `@value-change`<br />or `v-model` | `(valueChange)="handler($event)"` (text only)         | `onValueChange={(text, event) => …}`               |
| Native event      | _(same callback, 2nd arg)_   | _(same emit, 2nd arg)_            | `(change)="handler($event)"` (separate output)        | _(same callback, 2nd arg)_                         |
