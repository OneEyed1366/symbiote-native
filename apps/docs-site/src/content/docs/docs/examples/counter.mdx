...
| Press event | `onPress` directly on `View`   | `@press` directly on `View`    | `(press)` on a `Pressable` wrapper | `onPress` directly on `View`         | `onPress` directly on `View` |
...
`View` itself carries `onPress`/`onPressIn`/`onPressOut` — React, Vue,
Svelte, and Solid's simplest counter reaches for it directly rather than
wrapping in `Pressable`. Angular's counter uses `Pressable` here since its
non-function-children form is the idiomatic starting point on that adapter;
either works on every adapter (see the [Pressable example](/docs/examples/pressable/)).
